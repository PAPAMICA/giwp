...
/**
 * Widget MainWP Dashboard — détection de compromission GI-Toolkit (simple / détaillé).
 */
class MainWP_GIWeb_Compromise_Widget {

	const WIDGET_ID_SIMPLE   = 'mainwp-giweb-cd-widget-simple';
	const WIDGET_ID_DETAILED = 'mainwp-giweb-cd-widget-detailed';
	...
	public static function register_metabox( $metaboxes, $dashboard_siteid = null ) {
		$metaboxes = MainWP_GIWeb_Metabox::append(
			...
			__( 'GI-Toolkit — Compromission (simple)', 'mainwp-giweb' ),
		...
		return MainWP_GIWeb_Metabox::append(
			...
			__( 'GI-Toolkit — Compromission (détaillé)', 'mainwp-giweb' ),
	...
	public static function enqueue_assets( $hook ) {
		...
		wp_enqueue_style(
			'mainwp-giweb-cd-widget',
			MAINWP_GIWEB_PLUGIN_URL . 'assets/css/compromise-widget.css',
	...
	private static function render_network_metabox( $detailed ) {
		...
		$mod     = $detailed ? ' mainwp-giweb-cd-widget--detailed' : ' mainwp-giweb-cd-widget--simple';
		$theme   = $is_dark ? ' mainwp-giweb-cd-widget--dark' : ' mainwp-giweb-cd-widget--light';
		?>
		<div class="mainwp-giweb-cd-widget<?php echo esc_attr( $mod . $theme ); ?>">
	...
	public static function render_network_body( $detailed ) {
		...
		<div class="giweb-gw">
			...
			<?php if ( $detailed ) : ?>
				...
				<?php else : ?>
					<?php
					$list_mode = (string) ( MainWP_GIWeb_Settings::get()['cd_widget_list_mode'] ?? 'cards' );
					self::render_list_toolbar( $ctx, $list_mode );
					?>
					<div class="giweb-gw-list" data-default-view="<?php echo esc_attr( MainWP_GIWeb_Widget_UI::is_table_mode( $list_mode ) ? 'table' : 'cards' ); ?>" data-storage-key="giweb_gw_view_cd">
						<div class="giweb-gw-grid<?php echo esc_attr( MainWP_GIWeb_Widget_UI::list_view_class( $list_mode, 'cards' ) ); ?>">
							<?php foreach ( $ctx['rows'] as $row ) : ?>
								<?php self::render_cd_card( $row ); ?>
							<?php endforeach; ?>
						</div>
						<?php self::render_cd_table( $ctx['rows'], $list_mode ); ?>
					</div>
	...
	private static function get_network_context() {
		...
		return array(
			...
			'missing'        => $missing,
			'health'         => $health,
			'open_total'     => (int) ( $network['open_total'] ?? 0 ),
	...
	private static function render_network_header( array $ctx, $detailed = false ) {
		$subtitle = sprintf(
			...
			__( '%1$d / %2$d sites sains', 'mainwp-giweb' ),
			(int) $ctx['ok'],
		...
		MainWP_GIWeb_Widget_UI::render_header_row(
			'cd',
			__( 'Détection de compromission', 'mainwp-giweb' ),
			$subtitle,
			(int) $ctx['updated_at'],
			$empty_sync,
			array(
	...
	private static function render_network_overview( array $ctx ) {
		...
		$stats = array(
			array(
				...
				'label'    => __( 'Sains', 'mainwp-giweb' ),
				'modifier' => 'ok',
			),
			array(
				'strong'   => number_format_i18n( (int) $ctx['warn'] ),
				'label'    => __( 'Attention', 'mainwp-giweb' ),
				'modifier' => 'warn',
			),
			array(
	...
	private static function render_list_toolbar( array $ctx, $list_mode = 'cards' ) {
		...
		<div class="giweb-gw-toolbar">
			...
			<div class="giweb-gw-filters" role="tablist">
				...
				<button type="button" class="giweb-gw-filter" data-filter="ok" role="tab"><?php esc_html_e( 'Sains', 'mainwp-giweb' ); ?> <em><?php echo esc_html( (string) $ctx['ok'] ); ?></em></button>
			...
			<?php MainWP_GIWeb_Widget_UI::render_view_toggle( $list_mode, 'giweb_gw_view_cd' ); ?>
		</div>
		<?php
	}
	 ...
	 * @return void
	 */
	private static function render_cd_table( array $rows, $list_mode = 'cards' ) {
		?>
			...
			<table class="giweb-gw-table">
				<thead>
					<tr>
						...
						<th><?php esc_html_e( 'Dernier scan', 'mainwp-giweb' ); ?></th>
						<th><?php esc_html_e( 'Alertes', 'mainwp-giweb' ); ?></th>
				...
				<tbody>
					<?php foreach ( $rows as $row ) : ?>
						<tr data-status="<?php echo esc_attr( (string) ( $row['filter_status'] ?? 'issues' ) ); ?>" data-search="<?php echo esc_attr( (string) ( $row['search'] ?? '' ) ); ?>">
							...
							<td><?php echo esc_html( (string) ( $row['open_label'] ?? '—' ) ); ?></td>
	...
	public static function render_site_metabox( $site_id ) {
		...
		$is_dark = MainWP_GIWeb_UI::is_dark_theme();
		?>
		<div class="mainwp-giweb-cd-widget mainwp-giweb-cd-widget--single-site<?php echo $is_dark ? ' mainwp-giweb-cd-widget--dark' : ' mainwp-giweb-cd-widget--light'; ?>">
	...
	public static function render_site_body( $site_id ) {
		...
		$cd         = is_array( $row ) && isset( $row['compromise'] ) ? $row['compromise'] : MainWP_GIWeb_Compromise_Stats::get_site_compromise( $site_id );
		$updated_at = ! empty( $row['synced_at'] ) ? (int) $row['synced_at'] : (int) ( $agg['updated_at'] ?? 0 );
		$state      = MainWP_GIWeb_Compromise_Stats::get_visual_state( $cd );
		$admin_url  = is_array( $row ) ? self::site_cd_url( $row['url'] ?? '' ) : '';
		?>
		<div class="giweb-gw giweb-gw--single">
			...
			<?php if ( ! is_array( $cd ) ) : ?>
				<div class="giweb-gw-empty-state">
					<p><?php esc_html_e( 'Statut de compromission non remonté. Mettez à jour GI-Toolkit puis synchronisez le site.', 'mainwp-giweb' ); ?></p>
				</div>
			<?php else : ?>
				<?php
				self::render_cd_card(
					array(
						...
						'status_label'  => MainWP_GIWeb_Compromise_Stats::format_status_label( $cd ),
						'relative'      => MainWP_GIWeb_Compromise_Stats::format_relative_time( (int) ( $cd['last_scan'] ?? 0 ) ),
						'open_label'    => MainWP_GIWeb_Compromise_Stats::format_open_count( $cd ),
						'detail'        => self::card_detail( $cd ),
	 ...
	 * @return string
	 */
	private static function site_cd_url( $site_url ) {
		$site_url = untrailingslashit( (string) $site_url );
		if ( '' === $site_url ) {
			return '';
		}
		return $site_url . '/wp-admin/admin.php?page=gi-toolkit-settings-compromise-detection';
	}
	 ...
	 * @return string
	 */
	private static function card_detail( $cd ) {
		$types = MainWP_GIWeb_Compromise_Stats::format_types( $cd );
		$flags = MainWP_GIWeb_Compromise_Stats::format_flags( $cd );
		if ( '—' === $types && '—' === $flags ) {
			return '—';
		}
		if ( '—' === $types ) {
	 ...
	 * @return void
	 */
	private static function render_cd_card( array $row ) {
		...
		<article class="giweb-gw-card status-<?php echo esc_attr( $card_status ); ?>" data-status="<?php echo esc_attr( $filter_status ); ?>" data-search="<?php echo esc_attr( $search ); ?>">
			<header class="giweb-gw-card__head">
				...
				<span class="giweb-gw-card__meta"><?php echo esc_html( (string) ( $row['open_label'] ?? '—' ) ); ?></span>
			</header>
			...
			<div class="giweb-gw-card__rows">
				<div class="giweb-gw-card__row">
					<span class="giweb-gw-card__row-label"><?php esc_html_e( 'Dernier scan', 'mainwp-giweb' ); ?></span>
					<span class="giweb-gw-card__row-value"><?php echo esc_html( (string) ( $row['relative'] ?? '—' ) ); ?></span>
				</div>
				<div class="giweb-gw-card__row">
					<span class="giweb-gw-card__row-label"><?php esc_html_e( 'Détail', 'mainwp-giweb' ); ?></span>
					<span class="giweb-gw-card__row-value"><?php echo esc_html( (string) ( $row['detail'] ?? '—' ) ); ?></span>
				</div>
			</div>
		</article>
		<?php
	...
	private static function build_all_site_rows( $sites ) {
		...
		foreach ( MainWP_GIWeb_Sites::fetch_all( $mainwp_giweb_activator ) as $site ) {
			...
			$cd    = $site_row['compromise'] ?? null;
			$state = MainWP_GIWeb_Compromise_Stats::get_visual_state( is_array( $cd ) ? $cd : null );

			$rows[] = array(
				'label'         => $label,
				'admin_url'     => self::site_cd_url( $site_row['url'] ?? $normalized['url'] ?? '' ),
				'state'         => $state,
				'card_status'   => self::card_status_from_state( $state ),
				'filter_status' => 'ok' === $state ? 'ok' : 'issues',
				'status_label'  => MainWP_GIWeb_Compromise_Stats::format_status_label( is_array( $cd ) ? $cd : null ),
				'relative'      => MainWP_GIWeb_Compromise_Stats::format_relative_time( (int) ( is_array( $cd ) ? ( $cd['last_scan'] ?? 0 ) : 0 ) ),
				'open_label'    => MainWP_GIWeb_Compromise_Stats::format_open_count( is_array( $cd ) ? $cd : null ),
				'detail'        => self::card_detail( is_array( $cd ) ? $cd : null ),
	...
	private static function build_strip_segments( $agg_sites ) {
		...
		foreach ( MainWP_GIWeb_Sites::fetch_all( $mainwp_giweb_activator ) as $site ) {
			...
			$cd       = is_array( $site_row['compromise'] ?? null ) ? $site_row['compromise'] : null;
			$state    = MainWP_GIWeb_Compromise_Stats::get_visual_state( $cd );
			$status   = 'ok';
			if ( 'warn' === $state ) {
				$status = 'warn';
			...
			$detail       = self::card_detail( $cd );
			$tip          = sprintf(
				/* translators: 1: site, 2: status, 3: last scan, 4: open alerts, 5: detail */
				__( '%1$s — %2$s · scan %3$s · %4$s · %5$s', 'mainwp-giweb' ),
				$label,
				$status_label,
				$relative,
				$open_label,
			...
			$segments[] = array(
				...
				'tip_meta' => MainWP_GIWeb_Widget_UI::strip_tip_meta(
					...
					$status_label,
					array(
						MainWP_GIWeb_Widget_UI::strip_stat( __( 'Dernier scan', 'mainwp-giweb' ), $relative, $status ),
						MainWP_GIWeb_Widget_UI::strip_stat( __( 'Alertes', 'mainwp-giweb' ), $open_label, $status ),
						MainWP_GIWeb_Widget_UI::strip_stat( __( 'Détail', 'mainwp-giweb' ), $detail ),
					)
				),
			);
		}
