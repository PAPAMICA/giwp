...
class Activation {
	...
	public function create( $license_id ) {
		if ( empty( $license_id ) ) {
			return new \WP_Error( 'missing_key', __( 'Please enter a license key', 'gi-toolkit' ) );
		}
		...
		if ( empty( $activation->id ) ) {
			return new \WP_Error( 'could_not_activate', __( 'Could not activate the license.', 'gi-toolkit' ) );
		}
