...
class License {
	...
	public function get_current_release( $expires_in = 900 ) {
		...
		if ( empty( $key ) ) {
			return new \WP_Error( 'license_key_missing', __( 'Please enter a license key.', 'gi-toolkit' ) );
		}

		$activation_id = $this->client->settings()->activation_id;
		if ( empty( $activation_id ) ) {
			return new \WP_Error( 'activation_id_missing', __( 'This license is not yet activated.', 'gi-toolkit' ) );
		}
	...
	public function validate( $key, $store = false ) {
		...
		if ( is_wp_error( $license ) ) {
			if ( 'not_found' === $license->get_error_code() ) {
				throw new \Exception( esc_html__( 'This is not a valid license. Please double-check it and try again.', 'gi-toolkit' ), 404 );
			}
		...
		if ( 'revoked' === ( isset( $license->status ) ? $license->status : 'revoked' ) ) {
			throw new \Exception( esc_html__( 'This license has been revoked. Please re-purchase to obtain a new license.', 'gi-toolkit' ), 404 );
		}
	...
	public function validate_release() {
		...
		if ( empty( $current_release->release_json->slug ) || $this->client->slug !== $current_release->release_json->slug ) {
			throw new \Exception( esc_html__( 'This license is not valid for this product.', 'gi-toolkit' ) );
		}
	...
	public function validate_license( $license ) {
		if ( is_wp_error( $license ) ) {
			if ( $license->get_error_code( 'not_found' ) ) {
				return new \WP_Error( $license->get_error_code(), __( 'This license key is not valid. Please double check it and try again.', 'gi-toolkit' ) );
			}
			return $license;
		}
