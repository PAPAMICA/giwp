    ...
    public function validate()
    {
        ...
        while (!$this->end()) {
            ...
            if ($this->end()) {
                return new \WP_Error(
                    'parse_error',
                    __('Parse error: syntax error, unexpected end of snippet.', 'gi-toolkit'),
                    array(
                        'line' => $token[2],
                    ));
            }
                ...
                if ($this->check_duplicate_identifier($structure_type, $token[1])) {
                    switch ($structure_type) {
                        case T_FUNCTION:
                            ...
                            $message = __('Cannot redeclare function %s.', 'gi-toolkit');
                            break;
                        case T_CLASS:
                            /* translators: %s: PHP class name */
                            $message = __('Cannot redeclare class %s.', 'gi-toolkit');
                            break;
                        case T_INTERFACE:
                            /* translators: %s: PHP interface name */
                            $message = __('Cannot redeclare interface %s.', 'gi-toolkit');
                            break;
                        default:
                            /* translators: %s: PHP identifier name*/
                            $message = __('Cannot redeclare %s.', 'gi-toolkit');
                    }
