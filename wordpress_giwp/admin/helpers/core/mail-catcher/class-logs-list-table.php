...
class Gi_Toolkit_Mail_Catcher_Logs_List_Table extends WP_List_Table {
	...
	public function __construct() {
		self::$class_mail_catcher = new Gi_Toolkit_Mail_Catcher();
	...
	public function get_views() {
		...
		$statuses = array(
			0 => __( 'All', 'gi-toolkit' ),
            1 => __( 'Successful', 'gi-toolkit' ),
            2 => __( 'Failed', 'gi-toolkit' ),
		);
    ...
    public function search_box( $text, $input_id ) {
        ...
        <p class="search-box">
            ...
            <select name="search[place]">
				<option value="receiver" <?php selected( 'receiver', $search_place ); ?>><?php esc_html_e( 'Receiver', 'gi-toolkit' ); ?></option>
				<option value="subject" <?php selected( 'subject', $search_place ); ?>><?php esc_html_e( 'Subject', 'gi-toolkit' ); ?></option>
	...
	public function get_columns() {
		return array(
			...
			'id'         => __( 'ID', 'gi-toolkit' ),
			'receiver'   => __( 'Receiver', 'gi-toolkit' ),
			'subject'    => __( 'Subject', 'gi-toolkit' ),
			'mail_error' => __( 'Error', 'gi-toolkit' ),
			'time'       => __( 'Time', 'gi-toolkit' ),
			'actions'    => '',
		);
	}
		...
		foreach( $this->notices as $notice ) {
			...
			<div class="notice notice-<?php echo esc_attr( $notice['type'] ); ?> is-dismissible inline">
				...
				<button type="button" class="notice-dismiss">
					<span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'gi-toolkit' ); ?></span>
				</button>
			</div>
			<?php
		}
	...
	private function get_actions_html( $item ) {
		ob_start();
		?>
		<button class="gi-toolkit-view" type="button" name="view" value="<?php echo esc_attr( $item['id'] ); ?>">
			<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/eye.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
		</button>
		<button onclick="return confirm('<?php esc_html_e( 'Are you sure?', 'gi-toolkit' ); ?>')" class="button-link-delete" type="submit" name="delete" value="<?php echo esc_attr( $item['id'] ); ?>">
