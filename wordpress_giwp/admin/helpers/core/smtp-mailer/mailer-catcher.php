...
class Gi_Toolkit_SMTP_Mailer_Mailer_Catcher extends PHPMailer\PHPMailer\PHPMailer {
	...
	private function api_send() {
		...
		try {
			...
			// Get provider class
			$provider   = '';
			$class_name = 'Gi_Toolkit_SMTP_Mailer_Catcher_' . ucfirst( $active_provider );
			if ( class_exists( $class_name ) ) {
				$provider = new $class_name( $this );
			}

			if ( ! $provider ) {
				$this->throw_exception( esc_html__( 'The selected provider not found.', 'gi-toolkit' ) );
			}
