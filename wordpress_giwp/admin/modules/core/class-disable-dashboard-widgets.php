 ...
 * @since 1.3.0
 */
class Gi_Toolkit_Disable_Dashboard_Widgets {
    ...
    public function __construct() {
        ...
        $this->option_id        = GI_TOOLKIT_PLUGIN_SETTINGS . '_disable_dashboard_widgets';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Disable Dashboard Widgets', 'gi-toolkit' );
    }
    ...
    public function add_submenu(){
        ...
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-disable-dashboard-widgets',
    ...
    public function render_submenu() {
        ...
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/disable-dashboard-widgets.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/disable-dashboard-widgets.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/disable-dashboard-widgets.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e('Clean up and speed up the dashboard by completely disabling some or all widgets. Disabled widgets won\'t load any assets nor show up under Screen Options.', 'gi-toolkit'); ?></div>
                <div class="gi-toolkit__section__body">

                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e('Available widgets', 'gi-toolkit'); ?></div>
                            ...
                            <?php foreach ( $widgets as $widget_id => $widget ): ?>
                                <div class="gi-toolkit__checkbox">
                                    <label class="gi-toolkit__checkbox__label">
                                        ...
                                        <span class="gi-toolkit__checkbox__label__text"><?php echo esc_html($widget['title']); ?></span>
                                    </label>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    foreach ( $priorities as $priority => $widgets ) {
                        foreach( $widgets as $widget_id => $data ) {
                            ...
                            $dashboard_widgets[$widget_id] = array(
                                'id'        => $widget_id,
                                'title'     => isset( $data['title'] ) ? wp_strip_all_tags( preg_replace( '/ <span.*span>/im', '', $data['title'] ) ) : __( 'Widget(without name)', 'gi-toolkit' ),
                                'context'   => $context,
                                'priority'  => $priority,
                            );
                        }
