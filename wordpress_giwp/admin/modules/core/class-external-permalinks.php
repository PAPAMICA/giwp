 ...
 * @since 1.4.0
 */
class Gi_Toolkit_External_Permalinks {
    ...
    public function __construct() {
        $this->option_id    = GI_TOOLKIT_PLUGIN_SETTINGS . '_external_permalinks';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'External Permalinks', 'gi-toolkit' );
    }
    ...
    public function add_meta_boxes( $post_type ) {
        ...
        add_meta_box(
            'gi_toolkit-external-permalinks',
            esc_html__( 'External Permalinks', 'gi-toolkit' ),
    ...
    public function render_external_permalinks( $post ) {
        $value = get_post_meta( $post->ID, 'gi_toolkit_external_permalink', true );

        ?>
            <div>
                <input class="large-text" name="gi_toolkit_external_permalink" id="gi_toolkit_external_permalink" type="url" value="<?php echo esc_attr( $value ); ?>" />
                <p class="howto"><?php esc_html_e( 'The external permalink will open in a new browser tab, Leave this field empty to use the default WordPress permalink.', 'gi-toolkit' ); ?></p>
            </div>
        <?php
    }
    ...
    public function save_meta_boxes( $post_id ) {
        ...
        if ( isset( $_POST['gi_toolkit_external_permalink'] ) ) {

			//phpcs:ignore WordPress.Security.NonceVerification.Missing
            $external_permalink = trim( sanitize_url( wp_unslash( $_POST['gi_toolkit_external_permalink'] ) ) );

            if ( ! empty( $external_permalink ) ) {
                update_post_meta( $post_id, 'gi_toolkit_external_permalink', $external_permalink );
            } else {
                delete_post_meta( $post_id, 'gi_toolkit_external_permalink' );
            }
        }
    }
    ...
    public function use_external_permalink( $permalink, $post ) {
        ...
        $external_permalink = get_post_meta( $post->ID, 'gi_toolkit_external_permalink', true );
        if ( ! empty( $external_permalink ) && filter_var( $external_permalink, FILTER_VALIDATE_URL ) ) {
            $permalink = $external_permalink;

            if ( ! is_admin() ) { 
                $permalink = $permalink . '#gi_toolkit_new_tab';
            }
        }
    ...
    public function enqueue_scripts() {
        ...
        $external_permalink_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/external-permalinks-front.asset.php' );
        wp_enqueue_script( 'Gi_Toolkit_external_permalink_front', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/external-permalinks-front.js', $external_permalink_assets['dependencies'], $external_permalink_assets['version'], true );
		wp_localize_script( 'Gi_Toolkit_external_permalink_front', 'Gi_Toolkit_external_permalink_front', array(
    ...
    public function add_submenu(){
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-external-permalinks', 
    ...
    public function render_submenu() {
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/external-permalinks.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/external-permalinks.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/external-permalinks.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e( "Enable pages, posts and/or custom post types to have permalinks that point to external URLs. The rel=\"noopener noreferrer nofollow\" attribute will also be added for enhanced security and SEO benefits.", 'gi-toolkit'); ?></div>
                <div class="gi-toolkit__section__body">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Add \"target=_blank\"", 'gi-toolkit' ); ?></div>
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Add \"rel=noopener\"", 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content">
							<label class="gi-toolkit__toggle">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Add \"rel=noreferrer\"", 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content">
							<label class="gi-toolkit__toggle">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Add \"rel=nofollow\"", 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content">
							<label class="gi-toolkit__toggle">
