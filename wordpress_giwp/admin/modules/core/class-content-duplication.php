 ...
 * @since 1.4.0
 */
class Gi_Toolkit_Content_Duplication {
    ...
    public function __construct() {
        ...
        add_action( 'admin_action_gi_toolkit_content_duplication', array( $this, 'gi_toolkit_content_duplication' ) );
    }
    ...
    public function add_duplication_action_link( $actions, $post ) {
        ...
        $link = 'admin.php?action=gi_toolkit_content_duplication&amp;post=' . $post->ID . '&amp;nonce=' . wp_create_nonce( 'gi_toolkit_content_duplication_' . $post->ID ) . '';
        $actions['gi_toolkit-duplicate'] = sprintf( '<a href="%s">%s</a>', $link, __( 'Duplicate', 'gi-toolkit' ) );
     ...
     * @since 1.4.0
     */
    public function gi_toolkit_content_duplication() {

        if ( ! current_user_can( 'edit_posts' ) ) {
            wp_die( esc_html__( 'You do not have permission to perform this action.', 'gi-toolkit' ) );
        }
        ...
        if ( ! wp_verify_nonce( $nonce, 'gi_toolkit_content_duplication_' . $original_post_id ) ) {
            wp_die( esc_html__( 'Invalid nonce!', 'gi-toolkit' ) );
        }

        $original_post = get_post( $original_post_id );

        if ( ! $original_post ) {
            wp_die( esc_html__( 'Invalid post ID.', 'gi-toolkit' ) );
        }

        if ( in_array( $original_post->post_type, $this->excluded_posts ) ) {
            wp_die( esc_html__( 'You cannot duplicate this post type.', 'gi-toolkit' ) );
        }
        ...
        $args = array(
            ...
            'post_status'    => 'draft',
            'post_title'     => $original_post->post_title . ' (' . __( 'Duplicate', 'gi-toolkit' ) . ')',
