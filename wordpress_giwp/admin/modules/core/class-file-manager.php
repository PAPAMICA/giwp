...
class Gi_Toolkit_File_Manager {
    ...
    public function __construct() {
		$this->option_id    = GI_TOOLKIT_PLUGIN_SETTINGS . '_file_manager';
    ...
    public function class_init() {
		...
		$this->header_title = esc_html__( 'File Manager', 'gi-toolkit' );
		
		//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( ! is_admin() || ! isset( $_GET['page'] ) || $_GET['page'] != 'gi-toolkit-settings-file-manager' ) {
			return;
		}
    ...
    public function add_submenu(){
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-file-manager',
    ...
    public function render_submenu() {
		...
		$dependencies = array();
		wp_enqueue_script( 'Gi_Toolkit_submenu_jquery_datatables', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/jquery.dataTables.min.js', array('jquery'), '1.13.1', true );
		$dependencies[] = 'Gi_Toolkit_submenu_jquery_datatables';

		if ( 'upload' == $this->TEMPLATE ) {
			wp_enqueue_style( 'Gi_Toolkit_submenu_dropzone', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/dropzone-5.9.3.min.css', array(), '5.9.3', 'all' );
			wp_enqueue_script( 'Gi_Toolkit_submenu_dropzone', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/dropzone-5.9.3.min.js', array(), '5.9.3', true );
			$dependencies[] = 'Gi_Toolkit_submenu_dropzone';
		}

		if ( 'view' == $this->TEMPLATE ) {
			wp_enqueue_style( 'Gi_Toolkit_submenu_highlight', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/highlight-11.6.0.css', array(), '11.6.0', 'all' );
			wp_enqueue_script( 'Gi_Toolkit_submenu_highlight', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/highlight-11.6.0.js', array(), '11.6.0', true );
			$dependencies[] = 'Gi_Toolkit_submenu_highlight';
		}

		if ( 'edit' == $this->TEMPLATE ) {
			wp_enqueue_script( 'Gi_Toolkit_submenu_ace', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/ace-1.13.1-min.js', array(), '1.13.1', true );
			$dependencies[] = 'Gi_Toolkit_submenu_ace';

			//phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$ext = isset( $_GET['edit'] ) ? pathinfo( sanitize_text_field( wp_unslash( $_GET['edit'] ) ), PATHINFO_EXTENSION ) : '';
			$ext =  $ext == "js" ? "javascript" :  $ext;
			wp_localize_script( 'Gi_Toolkit_submenu_ace', 'Gi_Toolkit_submenu_ace', array(
				'ext'  => $ext,
				'i10n' => array(
					'bright'     => __( 'Bright', 'gi-toolkit' ),
					'dark'       => __( 'Dark', 'gi-toolkit' ),
					'errTimeout' => __( 'Error: Server Timeout', 'gi-toolkit' ),
					'errTry'     => __( 'Error: try again', 'gi-toolkit' ),
					'saved'      => __( 'Saved Successfully', 'gi-toolkit' ),
				),
			) );
		}
        ...
        wp_enqueue_style( 'Gi_Toolkit_submenu_bootstrap', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/bootstrap.min.css', array(), '5.3.0', 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu_bootstrap', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/lib/core/bootstrap.bundle.min.js', array(), '5.3.0', true );
		$dependencies[] = 'Gi_Toolkit_submenu_bootstrap';

        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/file-manager.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/file-manager.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/file-manager.js', $submenu_assets['dependencies'] + $dependencies, $submenu_assets['version'], true );
		wp_localize_script( 'Gi_Toolkit_submenu', 'Gi_Toolkit_FileManager', array(
			...
			'i10n'          => array(
				'oops'        => __( 'OOPS: minimum 3 characters required!', 'gi-toolkit' ),
				'noResults'   => __( 'No result found!', 'gi-toolkit' ),
				'errTrylater' => __( 'ERROR: Try again later!', 'gi-toolkit' ),
			),
		) );
	...
	public function show_admin_notice() {
		...
		$notice = gi_toolkit_clean( $_SESSION[$this->option_id]['notice'] ?? '' );
		...
		switch (true) {
			case isset( $_POST['ajax'], $_POST['token'] ) && isset( $_POST['type'] ):
				...
				if ( ! wp_verify_nonce( $nonce, $this->user_nonce ) ) {
                	...
                	die( esc_html__( 'Invalid token', 'gi-toolkit' ) );
				}
				...
				if ( $_POST['type'] == "save" ) {
					if ( ! is_dir( $this->PATH ) ) {
						wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' );
						exit;
					}
					...
					if ( $file == '' || ! is_file( $this->PATH . '/' . $file ) ) {
						$this->set_notice( __( 'File not found', 'gi-toolkit' ) );
						wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
						exit;
					}
					...
					if ( ! $wp_filesystem->put_contents( $file_path, $writedata, FS_CHMOD_FILE ) ){
						...
						die( esc_html__( 'Could Not Write File! - Check Permissions / Ownership', 'gi-toolkit' ) );
					}
					...
					try {
						if ( ! file_exists( $fullyQualifiedFileName ) ) {
							throw new Exception( sprintf( 
								...
								__( "File %s not found", 'gi-toolkit' ), 
								$fileName 
							) );
						}
						if ( copy( $fullyQualifiedFileName, $fullPath . $newFileName ) ) {
							echo esc_html( sprintf( 
								/* translators: %s: file name */
								__( "Backup %s created", 'gi-toolkit' ), 
								$newFileName 
							) );
						} else {
							throw new Exception( sprintf( 
								/* translators: %s: file name */
								__( "Could not copy file %s", 'gi-toolkit' ), 
								$fileName 
							) );
						}
					} catch (Exception $e) {
			...
			case isset( $_GET['del'], $_POST['token'] ):
				...
				$nonce = sanitize_text_field( wp_unslash( $_POST['token'] ?? '' ) );
				if ( ! wp_verify_nonce( $nonce, $this->user_nonce ) ) {
					$this->set_notice( __( 'Invalid token', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $del == '' || $del == '..' || $del == '.' ) {
					$this->set_notice( __( 'Invalid file or folder name', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $this->fm_rdelete( $this->PATH . '/' . $del ) ) {
					$this->set_notice( sprintf(
						...
						$is_dir ? __( "Folder <b>%s</b> Deleted", 'gi-toolkit' ) : __( "File <b>%s</b> Deleted", 'gi-toolkit' ), 
						$this->fm_enc( $del ) 
					), 'success' );
				} else {
					$this->set_notice( sprintf( 
						/* translators: %s: file or folder name */
						$is_dir ? __( "Folder <b>%s</b> not deleted", 'gi-toolkit' ) : __( "File <b>%s</b> not deleted", 'gi-toolkit' ), 
						$this->fm_enc( $del ) 
					) );
				}
			...
			case isset( $_POST['newfilename'], $_POST['newfile'], $_POST['token'] ):
				...
				if ( ! $this->fm_isvalid_filename( $new ) || $new == '' || $new == '..' || $new == '.' ) {
					$this->set_notice( __( 'Invalid characters in file or folder name', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $type == "file" ) {
					if ( ! file_exists( $this->PATH . '/' . $new ) ) {
						if ( $this->fm_is_valid_ext( $new ) ) {
							...
							if ( ! @fopen( $this->PATH . '/' . $new, 'w' ) ) {
								$this->set_notice( sprintf(
									/* translators: %s: file name */
									__( 'Cannot open file: %s', 'gi-toolkit' ), 
									$new 
								) );
								wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
								exit;
							...
							$this->set_notice( sprintf( 
								/* translators: %s: file name */
								__( 'File <b>%s</b> Created', 'gi-toolkit' ), 
								$this->fm_enc( $new ) 
							), 'success' );
						} else {
							$this->set_notice( __( 'File extension is not allowed', 'gi-toolkit' ) );
						}
					} else {
						$this->set_notice( sprintf( 
							/* translators: %s: file name */	
							__( 'File <b>%s</b> already exist', 'gi-toolkit' ), 
							$this->fm_enc( $new ) 
						) );
					}
				} else {
					if ( $this->fm_mkdir( $this->PATH . '/' . $new, false ) === true ) {
						$this->set_notice( sprintf( 
							...
							__( 'Folder <b>%s</b> Created', 'gi-toolkit' ), 
							$new 
						), 'success' );
					} elseif ( $this->fm_mkdir( $this->PATH . '/' . $new, false ) === $this->PATH . '/' . $new ) {
						$this->set_notice( sprintf( 
							/* translators: %s: folder name */
							__( 'Folder <b>%s</b> already exist', 'gi-toolkit' ), 
							$new 
						) );
					} else {
						$this->set_notice( sprintf( 
							/* translators: %s: folder name */
							__( 'Folder <b>%s</b> not created', 'gi-toolkit' ), 
							$new 
						) );
					}
				}
			...
			case isset( $_GET['copy'], $_GET['finish'] ):
				...
				if ( $copy == '' ) {
					$this->set_notice( __( 'Source path not defined', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $from != $dest ) {
					...
					if ( $move ) {
						...
						if ( $rename ) {
							$message = sprintf( 
								...
								__( "Moved from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
								$this->fm_enc( $copy ), 
							...
							$this->set_notice(
								__( "File or folder with this path already exists", 'gi-toolkit' ) 
							);
						} else {
							$message = sprintf( 
								/* translators: 1: source path, 2: destination path */
								__( "Error while moving from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
								$this->fm_enc( $copy ), 
								$this->fm_enc( $msg_from ) 
							);
							$this->set_notice( $message, 'success' );
						...
						if ( $this->fm_rcopy( $from, $dest ) ) {
							$message = sprintf( 
								/* translators: 1: source path, 2: destination path */
								__( "Copied from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
								$this->fm_enc( $copy ), 
								$this->fm_enc( $msg_from ) 
							);
							$this->set_notice( $message, 'success' );
							...
							$message = sprintf( 
								/* translators: 1: source path, 2: destination path */
								__( "Error while copying from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
								$this->fm_enc( $copy ), 
								$this->fm_enc( $msg_from ) 
							);
					...
					if ( ! $move ){
						...
						$this->set_notice( __( 'Source and destination paths must be not equal', 'gi-toolkit' ) );
					}
			...
			case isset( $_POST['file'], $_POST['copy_to'], $_POST['finish'], $_POST['token'] ):
				...
				if ( $this->PATH == $copy_to_path ) {
					$this->set_notice( __( 'Paths must be not equal', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( ! is_dir( $copy_to_path ) ) {
					if ( ! $this->fm_mkdir( $copy_to_path, true ) ) {
						$this->set_notice( __( 'Unable to create destination folder', 'gi-toolkit' ) );
						wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
						exit;
					}
				}
				...
				$errors        = 0;
				//phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$files_to_copy = gi_toolkit_clean( wp_unslash( $_POST['file'] ?? array() ) );

				if ( ! is_array( $files_to_copy ) || ! count( $files_to_copy ) ) {
					$this->set_notice( __( 'Nothing selected', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $errors > 0 ) {
					$message = $move ? __( 'Error while moving items', 'gi-toolkit' ) : __( 'Error while copying items', 'gi-toolkit' );
					$this->set_notice( $message );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}

				$message = $move ? __( 'Selected files and folders moved', 'gi-toolkit' ) : __( 'Selected files and folders copied', 'gi-toolkit' );
			...
			case isset( $_POST['rename_from'], $_POST['rename_to'], $_POST['token'] ):
				...
				if ( ! $this->fm_isvalid_filename( $new ) || $old == '' || $new == '' ) {
					$this->set_notice( __( 'Invalid characters in file name', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $this->fm_rename( $this->PATH . '/' . $old, $this->PATH . '/' . $new ) ) {
					$message = sprintf( 
						...
						__( "Renamed from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
						$this->fm_enc( $old ), 
						$this->fm_enc( $new ) 
					);
					$this->set_notice( $message, 'success' );
					...
					$message = sprintf( 
						/* translators: 1: old file name, 2: new file name */
						__( "Error while renaming from <b>%1\$s</b> to <b>%2\$s</b>", 'gi-toolkit' ), 
						$this->fm_enc( $old ), 
						$this->fm_enc( $new ) 
					);
					$this->set_notice( $message, 'success' );
			...
			case ! empty( $_FILES ):
				...
				$nonce = sanitize_text_field( wp_unslash( $_POST['token'] ?? '' ) );
				if ( ! wp_verify_nonce( $nonce, $this->user_nonce ) ) {
					wp_send_json( array( 'status' => 'error', 'info' => esc_html__( 'Invalid Token', 'gi-toolkit' ) ) );
				}
				...
				$response      = array (
					'status' => 'error',
					'info'   => __( 'Oops! Try again', 'gi-toolkit' ),
				);
				...
				if ( ! $this->fm_isvalid_filename( $filename ) && ! $this->fm_isvalid_filename( $fullPathInput ) ) {
					wp_send_json( array( 'status' => 'error', 'info' => __( 'Invalid File name!', 'gi-toolkit' ) ) );
				}
					...
					if ( $chunkTotal ){
						...
						if ( $chunkContent !== false ) {
							...
							if ( $success ) {
								$response = array(
									'status' => 'success',
									'info'   => __( 'File upload successful', 'gi-toolkit' ),
								);
							} else {
								$response = array(
									'status' => 'error',
									'info'   => __( 'Failed to write file', 'gi-toolkit' ),
								);
							}
						} else {
							$response = array(
								'status'       => 'error',
								'info'         => __( 'Failed to read input file', 'gi-toolkit' ),
						...
						if ( $wp_filesystem->exists( $file_fullPath ) ) {
							$response = array (
								'status' => 'success',
								'info'   => __( 'file upload successfully', 'gi-toolkit' ),
							);
						} else {
							$response = array (
								'status' => 'error',
								'info'   => __( 'Couldn\'t upload the requested file.', 'gi-toolkit' ),
							);
						}
					} else {
						$response = array (
							'status' => 'error',
							'info'   => __( 'Error while uploading files.', 'gi-toolkit' ),
						);
					}
				}
			...
			case isset( $_POST['group'], $_POST['delete'], $_POST['token'] ):
				...
				$errors          = 0;
				//phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$files_to_delete = gi_toolkit_clean( wp_unslash( $_POST['file'] ?? array() ) );

				if ( ! is_array( $files_to_delete ) || ! count( $files_to_delete ) ) {
					$this->set_notice( __( 'No files selected', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
				...
				if ( $errors == 0 ) {
					$this->set_notice( __( 'Selected files and folders deleted', 'gi-toolkit' ), 'success' );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				} else {
					$this->set_notice( __( 'Error while deleting items', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
			break;
			case isset( $_POST['group'], $_POST['token'] ) && ( isset( $_POST['zip'] ) || isset( $_POST['tar'] ) ):
				...
				if ( ( $ext == "zip" && ! class_exists('ZipArchive') ) || ( $ext == "tar" && ! class_exists('PharData') ) ) {
					$this->set_notice( __( 'Operations with archives are not available', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}

				//phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$files_to_zip    = gi_toolkit_clean( wp_unslash( $_POST['file'] ?? array() ) );
				...
				if ( $ext == 'zip' ) {
					include GI_TOOLKIT_PLUGIN_PATH . '/admin/helpers/core/file-manager/class-fm-zipper.php';
					$zipper = new Gi_Toolkit_FM_Zipper();
					$res    = $zipper->create( $zipname, $files_to_zip );
				} elseif ( $ext == 'tar' ) {
					include GI_TOOLKIT_PLUGIN_PATH . '/admin/helpers/core/file-manager/class-fm-zipper-tar.php';
					$tar = new Gi_Toolkit_FM_Zipper_Tar();
				...
				if ( $res ) {
					$message = sprintf( 
						...
						__( "Archive <b>%s</b> Created", 'gi-toolkit' ), 
						$this->fm_enc( $zipname ) 
					);
					$this->set_notice( $message, 'success' );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					...
					$this->set_notice( __( 'Archive not created', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
			...
			case isset( $_POST['unzip'], $_POST['token'] ):
				...
				if ( $res ) {
					$this->set_notice( __( 'Unzip successful', 'gi-toolkit' ), 'success' );
				} else {
					$this->set_notice( __( 'Unzip failed', 'gi-toolkit' ) );
				}
			...
			case isset( $_POST['chmod'], $_POST['token'] ) && ! $this->FM_IS_WIN:
				...
				if ( $wp_filesystem->chmod( $path . '/' . $file, $mode ) ) {
					$this->set_notice( __( 'Permissions changed', 'gi-toolkit' ), 'success' );
				} else {
					$this->set_notice( __( 'Permissions not changed', 'gi-toolkit' ) );
				}
			...
			case isset( $_POST['copy'] ):
				...
				//phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
				$copy_files     = isset( $_POST['file'] ) ? gi_toolkit_clean( wp_unslash( $_POST['file'] ) ) : null;
				$this->TEMPLATE = 'copy';
			...
			case isset( $_GET['edit'] ):
				...
				if ( in_array( $ext, $this->fm_get_text_exts() ) || substr( $mime_type, 0, 4 ) == 'text' || in_array( $mime_type, $this->fm_get_text_mimes() ) ) {
					...
					$this->set_notice(  __( 'File extension not supported for editing', 'gi-toolkit' ) );
					wp_safe_redirect( $this->FM_SELF_URL . '?page=gi-toolkit-settings-file-manager&p=' . urlencode( $this->FM_PATH ) );
					exit;
				}
			break;
        ...
        ?>
            <div class="gi-toolkit__section">
                ...
                <div class="gi-toolkit__section__body">
						...
						<?php
							switch ( $this->TEMPLATE ) {
								case 'upload':
									...
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/upload.php';
								break;
								case 'copyfinish':
									//phpcs:ignore WordPress.Security.NonceVerification.Recommended
									$copy           = sanitize_text_field( wp_unslash( $_GET['copy'] ?? '' ) );
									...
									$this->filemanger_navbar();
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/copyfinish.php';
								break;
								case 'copy':
									//phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
									$copy_files = isset( $_POST['file'] ) ? gi_toolkit_clean( wp_unslash( $_POST['file'] ) ) : null;
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/copy.php';
								break;
								case 'view':
									//phpcs:ignore WordPress.Security.NonceVerification.Recommended
									$file            = sanitize_text_field( wp_unslash( $_GET['view'] ?? '' ) );
								...
								case 'edit':
									...
									$this->filemanger_navbar();
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/edit.php';
								break;
								case 'chmod':
									//phpcs:ignore WordPress.Security.NonceVerification.Recommended
									$file      = sanitize_text_field( wp_unslash( $_GET['chmod'] ?? '' ) );
									...
									$this->filemanger_navbar();
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/chmod.php';
								break;
								default:
									...
									$this->filemanger_navbar();
									include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/default.php';
								break;
							}
						?>
					</section>
					...
					<?php
						include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/modals/confirm.php';
						include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/modals/new-item.php';
						include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/modals/rename.php';
						include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/modals/search.php';
					?>
	...
	private function set_global_variables() {
		...
		if ( ! $this->is_child_path( $this->PATH, ABSPATH ) ) {
			wp_die( esc_html__( 'Sorry, you are not allowed to access this page.', 'gi-toolkit' ) );
		}
	}
	...
	private function filemanger_navbar() {
        ...
        $root_url   = "<a href='?page=gi-toolkit-settings-file-manager'><i class='fa fa-home' aria-hidden='true' title='" . $this->FM_ROOT_PATH . "'></i></a>";
        ...
        if ( $path != '' ) {
            ...
            for ( $i = 0; $i < $count; $i++ ) {
                ...
                $array[]    = "<a href='?page=gi-toolkit-settings-file-manager&token=". $this->TOKEN ."&p={$parent_enc}'>" . $this->fm_enc( $this->fm_convert_win( $exploded[$i] ) ) . "</a>";
            }
            $root_url .= $sep . implode( $sep, $array );
        }

		include GI_TOOLKIT_PLUGIN_PATH . '/admin/templates/core/file-manager/views/navbar.php';
    }
    ...
    */
    private function fm_download_file( $fileLocation, $fileName, $chunkSize  = 1024 ) {
        ...
        if ( $size == 0 ) {
            $result = array(
                'status' => false,
                'msg'    => __( 'Zero byte file! Aborting download', 'gi-toolkit' ),
            );
            return $result;
        }
        ...
        if ( $fp === false ) {
            $result = array(
                'status' => false,
                'msg'    => __( 'Cannot open file! Aborting download', 'gi-toolkit' ),
            );
            return $result;
        }
