 ...
 * @since 1.11.0
 */
class Gi_Toolkit_Local_Avatars {
	private $option_id;
    private $user_nonce;
    ...
    public function __construct() {
		$this->option_id            = GI_TOOLKIT_PLUGIN_SETTINGS . '_local_avatars';
	...
	function enqueue_scripts_styles( $hook_suffix ) {
		if ( 'profile.php' == $hook_suffix || 'user-edit.php' == $hook_suffix ) {
			wp_enqueue_media();
			$metabox_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/local-avatars-metabox.asset.php' );
			wp_enqueue_style( 'Gi_Toolkit_local_avatars_metabox', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/local-avatars-metabox.css', array(), $metabox_assets['version'], 'all' );
			wp_enqueue_script( 'Gi_Toolkit_local_avatars_metabox', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/local-avatars-metabox.js', $metabox_assets['dependencies'], $metabox_assets['version'], true );
		}
	}
	...
	public function render_user_meta_box( $user ) {
		...
		?>
		<div class="gi_toolkit-local-avatars">
			<table class="form-table">
				<tr>
					<th><label><?php esc_html_e( 'Profile Picture', 'gi-toolkit' ); ?></label></th>
					<td>
						<div class="site-icon-section">
							<div id="gi_toolkit-local-avatar-icon-preview-container">
								<?php //phpcs:ignore PluginCheck.CodeAnalysis.ImageFunctions.NonEnqueuedImage ?>
								<img id="gi_toolkit-local-avatar-icon-preview" class="app-icon-preview" src="<?php echo esc_url( $avatar_url ); ?>">
							</div>
							<input type="hidden" name="gi_toolkit_local_avatar_icon" id="gi_toolkit_local_avatar_icon_hidden_field" value="<?php esc_attr( $avatar_id ); ?>" />
							<div class="action-buttons">
								<button type="button"
									id="gi_toolkit-local-avatar-choose-from-library-button"
									type="button"
									class="<?php echo esc_attr( $classes_for_button ); ?>"
									data-alt-classes="<?php echo esc_attr( $classes_for_button_on_change ); ?>"
									data-size="100"
									data-choose-text="<?php esc_attr_e( 'Choose Image', 'gi-toolkit' ); ?>"
									data-update-text="<?php esc_attr_e( 'Change Image', 'gi-toolkit' ); ?>"
									data-update="<?php esc_attr_e( 'Set as Image', 'gi-toolkit' ); ?>"
									data-state="<?php echo esc_attr( $has_avatar ); ?>"
								>
									<?php if ( $has_avatar ) : ?>
										<?php esc_html_e( 'Change Image', 'gi-toolkit' ); ?>
									<?php else : ?>
										<?php esc_html_e( 'Choose an Image', 'gi-toolkit' ); ?>
									<?php endif; ?>
								</button>
								<button
									id="gi_toolkit-js-remove-local-avatar-icon"
									type="button"
									<?php echo $has_avatar ? 'class="button button-secondary reset"' : 'class="button button-secondary reset hidden"'; ?>
								>
									<?php esc_html_e( 'Remove Image', 'gi-toolkit' ); ?>
								</button>
							</div>
						</div>
	...
	public function save_user_meta_box( $user_id ) {
		...
		$avatar_id = sanitize_text_field( wp_unslash( $_POST['gi_toolkit_local_avatar_icon'] ?? '' ) );
		if ( $avatar_id ) {
