 ...
 * @updated 1.3.0
 */
class Gi_Toolkit_Blacklisted_Usernames {

    const ACTION    = 'gi_toolkit-blacklisted-usernames-action';
    const NONCE     = 'gi_toolkit-blacklisted-usernames-nonce';
    ...
    public function show_notice_to_change_username() {
        ...
        $assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/blacklisted-usernames.asset.php' );
        wp_enqueue_style( 'gi_toolkit-blacklisted-usernames-admin', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/blacklisted-usernames.css', array(), $assets['version'], 'all' );
        wp_enqueue_script( 'gi_toolkit-blacklisted-usernames-admin', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/blacklisted-usernames.js', $assets['dependencies'], $assets['version'], true );
        wp_localize_script( 'gi_toolkit-blacklisted-usernames-admin', 'gi_toolkitBlacklistedUsernames', array(
        ...
        ?>
            <div class="notice notice-error" id="gi_toolkit-blacklisted-usernames-notice">
                <div id="gi_toolkit-blacklisted-usernames-text">
                    <p>
                        <strong>Gi_Toolkit: </strong>
                        <?php 
                        echo esc_html( sprintf(
                            /* translators: %s: username */
                            __( 'You can\'t use "%s" as your username. It\'s not safe. Please pick another username for better security.', 'gi-toolkit' ), 
                            $this->last_username 
                        ) ); 
                        ?>
                    </p>
                    <p><?php esc_html_e( 'After changing the username you\'ll need to login again.', 'gi-toolkit' ) ?></p>
                </div>
                <input type="text" id="gi_toolkit-blacklisted-usernames-input" placeholder="<?php esc_attr_e( 'Enter new username', 'gi-toolkit' ); ?>">
                <input type="submit" id="gi_toolkit-blacklisted-usernames-button" class="button button-primary" value="<?php esc_attr_e( 'Change', 'gi-toolkit'); ?>">
                <p id="gi_toolkit-blacklisted-usernames-message"></p>
            </div>
        <?php
    }
    ...
    public function change_admin_name() {
        ...
        if ( ! current_user_can( 'manage_options' ) || ! current_user_can( 'list_users' ) ) {
            wp_send_json_error( array( 'message' => __( 'You do not have permission to perform this action.', 'gi-toolkit' ) ) );
        }

        $nonce    = sanitize_text_field( wp_unslash( $_POST['nonce'] ?? '' ) );
        $username = sanitize_text_field( wp_unslash( $_POST['username'] ?? '' ) );
        if ( ! wp_verify_nonce( $nonce, self::NONCE ) || empty( $username ) ) {
            wp_send_json_error( array( 'message' => __( 'Refresh the page and try again.', 'gi-toolkit' ) ) );
        }

        $newusername = sanitize_text_field( wp_unslash( $_POST['newusername'] ?? '' ) );
        $newusername = sanitize_title( trim( $newusername ) );
        if ( empty( $newusername ) ) {
            wp_send_json_error( array( 'message' => __( 'Username can\'t be empty.', 'gi-toolkit' ) ) );
        }

        if ( in_array( $newusername, self::BLACKLIST ) ) {
            wp_send_json_error( array( 'message' => __( 'Sorry, that username is not allowed.', 'gi-toolkit' ) ) );
        }
        ...
        if ( username_exists( $newusername ) ) {
            wp_send_json_error( 
                array(
                 'message' => sprintf(
                        /* translators: %s: username */
                        __( 'The username "%s" already exists. Please choose a different one.', 'gi-toolkit' ), 
                        $newusername 
                    ) 
                ) 
            );
        ...
        if ( ! $admin_user ) {
            wp_send_json_error( 
                array( 'message' => sprintf( 
                        /* translators: %s: username */
                        __( 'No user has the username "%s". Nothing to update.', 'gi-toolkit' ), 
                        $username
                    ) 
                ) 
            );
        ...
        if ( ! in_array( 'administrator', $user_roles ) ) {
            wp_send_json_error( array( 'message' => __( 'This action can only be performed on administrator accounts.', 'gi-toolkit' ) ) );
        }
        ...
        if ( get_user_by( 'login', $newusername ) ) {
            wp_send_json_success( array( 'message' => __( 'Username changed successfully. Please logout and login with the new username.', 'gi-toolkit' ) ) );
        }

        wp_send_json_error( array( 'message' => __( 'Username change failed.', 'gi-toolkit' ) ) );
    }
}
