 ...
 * @since 1.0.0
 */
class Gi_Toolkit_Move_Login_URL {
    ...
    public function __construct() {
        ...
        $this->option_id   	= GI_TOOLKIT_PLUGIN_SETTINGS . '_move_login_url';
    ...
    public function class_init() {
		$this->header_title	= esc_html__( 'Move login URL', 'gi-toolkit' );
    }
    ...
    public static function activate() {
		...
		$settings = get_option( GI_TOOLKIT_PLUGIN_SETTINGS . '_move_login_url', array() );
...
location ^~ /wp-admin {
	...
	public function new_login_url( $scheme = null ) {
		 ...
		 */
		$url = apply_filters( 'gi_toolkit/move_login_url/home_url', home_url( '/', $scheme ) );
	...
	public function plugins_loaded() {
		...
		/**
		 * gi_toolkit/move_login_url/signup_enable use this filter to enable or disable the signup feature.
		 *
		 * @since 1.0.0
		 *
		...
		if ( ! is_multisite()
		    ...
		    || strpos( rawurldecode( $request_uri ), 'wp-activate' ) !== false ) && apply_filters( 'gi_toolkit/move_login_url/signup_enable', false ) === false ) {

			wp_die( esc_html__( 'This feature is not enabled.', 'gi-toolkit' ), 403 );
	...
	public function setup_theme() {
		...
		if ( ! is_user_logged_in() && 'customize.php' === $pagenow ) {
			wp_die( esc_html__( 'This has been disabled', 'gi-toolkit' ), 403 );
		}
	}
	...
	public function wp_loaded() {
		 ...
		 */
		do_action( 'gi_toolkit/move_login_url/before_redirect', $request );
			...
			if ( $pagenow === 'wp-login.php' && isset( $request['path'] ) && $request['path'] !== $this->user_trailingslashit( $request['path'] ) && get_option( 'permalink_structure' ) ) {
				...
				if ( is_user_logged_in() ) {
					...
					if ( ! isset( $_REQUEST['action'] ) ) {
						 ...
						 */
						$logged_in_redirect = apply_filters( 'gi_toolkit/move_login_url/logged_in_redirect', $redirect_to, $requested_redirect_to, $user );
    ...
    public function add_submenu(){
        ...
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-move-login-url', 
    ...
    public function render_submenu() {
        ...
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/move-login-url.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/move-login-url.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/move-login-url.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
        wp_localize_script( 'Gi_Toolkit_submenu', 'gi-toolkit', array(
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e( 'Change the login URL and prevent access to the wp-login.php page and the wp-admin directory to non-connected people.', 'gi-toolkit' ); ?></div>
                <div class="gi-toolkit__section__body">
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Login URL', 'gi-toolkit' ); ?></div>
                    ...
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Redirection URL', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">
                            <div class="gi-toolkit__input-text slug-url">
                                <div>
									<code>
			...
			<div class="gi-toolkit__section">
                <div class="gi-toolkit__section__body">
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e("403 for not logged in on /wp-admin", 'gi-toolkit'); ?></div>
						<div class="gi-toolkit__section__body__item__content">
							<label class="gi-toolkit__toggle">
								<input type="hidden" name="<?php echo esc_attr( $this->option_id . '[wp_admin_403]' ); ?>" value="0">
								<input type="checkbox" name="<?php echo esc_attr( $this->option_id . '[wp_admin_403]' ); ?>" value="1" <?php checked( $wp_admin_403, '1' ); ?>>
								<span class="gi-toolkit__toggle__slider round"></span>
							</label> 
						</div>
						<div class="gi-toolkit__section__body__item__desc">
							<?php esc_html_e("Prevent access to ^/wp-admin for not logged in users with a 403 error. This feature allows you to block the request directly via Apache or Nginx, which saves your server resources. If this feature is enabled the redirection URL will be ignored.", 'gi-toolkit'); ?>
						</div>
                    </div>
				</div>
			</div>
