 ...
 * @since 2.17.0
 */
class Gi_Toolkit_Search_Replace_In_Database {
    ...
    public function __construct() {
		...
		$this->option_id       = GI_TOOLKIT_PLUGIN_SETTINGS . '_search_replace_in_database';
		...
		$this->page_id	       = 'gi-toolkit-settings-search-replace-in-database';
		$this->max_free        = 10;

		add_action( 'init', array( $this, 'class_init' ) );
        add_action( 'admin_menu', array( $this, 'add_submenu' ), 999 );
		add_action( 'wp_ajax_gi_toolkit_search_replace_in_database_start', array( $this, 'ajax_search_replace_in_database_start' ) );
	}
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Search Replace in database', 'gi-toolkit' );
    }
    ...
    public function render_submenu() {
        ...
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/search-replace-in-database.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/search-replace-in-database.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/search-replace-in-database.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
		wp_localize_script( 'Gi_Toolkit_submenu', 'Gi_Toolkit_search_replace_in_database', array(
			'ajax_url'          => admin_url( 'admin-ajax.php' ),
			'nonce'             => wp_create_nonce( $this->nonce_name ),
			'max_free'          => $this->max_free,
			'dry_run_default'   => Gi_Toolkit_Security::get_option( 'search_replace_dry_run_default' ),
			'i18n'     => array(
				'search_required'        => esc_html__( 'Please enter a search term.', 'gi-toolkit' ),
				'select_table'           => esc_html__( 'Please select at least one table.', 'gi-toolkit' ),
				'something_wrong'        => esc_html__( 'Something went wrong. Please try again.', 'gi-toolkit' ),
				'run_status_progression' => esc_html__( 'Progression', 'gi-toolkit' ),
				'run_status_completed'   => esc_html__( 'Done', 'gi-toolkit' ),
			),
		) );
	...
	public function ajax_search_replace_in_database_start(){
		gi_toolkit_ajax_require_cap( 'manage_options', $this->nonce_name );
		...
		if ( 0 === $step && 0 === $page ) {
			...
			if ( isset( $args['tables'] ) && ! is_array( $args['tables'] ) ) {
				$args['tables'] = explode( ' ', (string) $args['tables'] );
			}
			if ( empty( $args['tables'] ) || ! is_array( $args['tables'] ) ) {
				wp_send_json_error( array(
					'message'      => __( 'Please select at least one table.', 'gi-toolkit' ),
					'message_type' => 'danger',
				) );
			}
		...
		foreach ( $data as $row ) {
			...
			foreach( $columns as $column ) {
				...
				if ( $wpdb->options === $table ) {
					...
					// Skip any GI-Toolkit options as they may contain the search field.
			...
			if ( '1' === $args['run_dry'] ) {
				...
				if ( ! $result ) {
					...
					$table_report['errors'][] = sprintf( __( 'Error updating row: %d.', 'gi-toolkit' ), $current_row );
				} else {
					$table_report['updates']++;
				}
			}
    ...
    private function submenu_content() {
		...
		?>
			<div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e( "Search and replace data in your WordPress database safely and efficiently.", 'gi-toolkit'); ?></div>
                <div class="gi-toolkit__section__body">

					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__content" id="JS-search-replace-items">
							<?php $this->search_replace_item( 1, true ); ?>
						</div>

						<div class="gi-toolkit__button search-replace-add-row gi-toolkit__tooltip__container">
							<button type="button" class="secondary gi-toolkit__tooltip__trigger <?php echo esc_attr( ! $is_pro ? 'disabled' : '' ); ?>" id="JS-search-replace-add-row">
								<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/plus.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
								<?php esc_html_e( 'Add row', 'gi-toolkit' ); ?>
							</button>

							<?php if ( ! $is_pro ): ?>
							<div class="gi-toolkit__tooltip">
								<div class="gi-toolkit__tooltip__text"><?php esc_html_e( 'To go further, upgrade to the version', 'gi-toolkit' ); ?></div>
								<div class="gi-toolkit__tooltip__triangle"></div>
							</div>
							<?php endif;?>
						</div>
					...
					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Select Tables", 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content select-tables">
							<div class="gi-toolkit__section__body__item__content__select-tables" id="JS-tables-container">
								<div class="gi-toolkit__checkbox">
									<label class="gi-toolkit__checkbox__label">
										<input type="checkbox" id="JS-select-all-tables">
										<span class="mark"></span>
										<span class="gi-toolkit__checkbox__label__text"><?php esc_html_e( "Select All", 'gi-toolkit' ); ?></span>
									</label>
								</div>
								...
								?>
									<div class="gi-toolkit__checkbox">
										<label class="gi-toolkit__checkbox__label">
											...
											<span class="mark"></span>
											<span class="gi-toolkit__checkbox__label__text">
					...
					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__title">
							<?php esc_html_e( 'Max Page Size', 'gi-toolkit' ); ?>
						</div>
						<div class="gi-toolkit__section__body__item__content">
							...
							<div class="description"><?php esc_html_e( 'If you notice timeouts or are unable to backup/import the database, try decreasing this value.', 'gi-toolkit' ); ?></div>
						</div>
					</div>
						...
						<div class="gi-toolkit__section__body__item__title activable">
							<div>
								<label class="gi-toolkit__toggle">
									<input class="gi-toolkit__sortable__item__hide" type="checkbox" id="JS-replace-guids">
									<span class="gi-toolkit__toggle__slider round"></span>
								</label>
							</div>
							<div><?php esc_html_e( "Replace GUIDs", 'gi-toolkit' ); ?></div>
						</div>
						<div class="gi-toolkit__section__body__item__content">
							<div><?php esc_html_e( "Activate this option to include 'guid' columns in the search and replace operation.", 'gi-toolkit' ); ?></div>
						</div>
					</div>

					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__content">
                        	<div class="gi-toolkit__section__body__item__content__buttons">
								<div class="gi-toolkit__button search-replace-dry-run">
									<button type="button" class="secondary" id="JS-search-replace-dry-run">
										<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/flask.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
										<?php esc_html_e( 'Run as dry run', 'gi-toolkit' ); ?>
										<span class="spinner"></span>
									</button>
								</div>

								<div class="gi-toolkit__button search-replace-run">
									<button type="button" class="primary" id="JS-search-replace-real-run">
										<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/replace.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
										<?php esc_html_e( 'Search & Replace', 'gi-toolkit' ); ?>
										<span class="spinner"></span>
									</button>
								</div>
							</div>
			...
			<div class="gi-toolkit__section search-replace-dry-run-section" id="JS-search-replace-dry-run-section">
				<div class="gi-toolkit__section__body">
					<div class="gi-toolkit__section__body__header">
						<div class="gi-toolkit__section__body__header__title">
							<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/flask.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
							<?php esc_html_e( 'Dry run', 'gi-toolkit' ); ?>
						</div>
						<div class="gi-toolkit__section__body__header__status" id="JS-search-replace-dry-run-status">(<?php esc_html_e( 'Progression', 'gi-toolkit' ); ?>)</div>
					</div>
					<div class="gi-toolkit__section__body__body">
						<div class="gi-toolkit__section__body__body__top">
							<div class="gi-toolkit__section__body__body__data">
								<div class="gi-toolkit__section__body__body__data__percentage" id="JS-search-replace-dry-run-percentage">0%</div>
								<div class="gi-toolkit__section__body__body__data__size" id="JS-search-replace-dry-run-size">(0 MB / 0 MB)</div>
								<div class="gi-toolkit__section__body__body__data__time" id="JS-search-replace-dry-run-time">00:00:00</div>
							</div>
							<div class="gi-toolkit__section__body__body__actions">
								<span class="gi-toolkit__section__body__body__actions__stop" id="JS-search-replace-dry-run-stop">
									<span class="gi-toolkit__section__body__body__actions__stop__icon"></span>
									<?php esc_html_e( 'Stop', 'gi-toolkit' ); ?>
								</span>
							</div>
						</div>
						<div class="gi-toolkit__section__body__body__bottom">
							<div class="gi-toolkit__section__body__body__progress-bar">
								<div class="gi-toolkit__section__body__body__progress-bar__fill" id="JS-search-replace-dry-run-progress-bar"></div>
							</div>
						</div>
					</div>
					<div class="gi-toolkit__section__body__footer">
						<?php
							echo wp_kses_post( sprintf(
								/* translators: 1: tables, 2: cells, 3: changes */
								__( '%1$s tables were searched, %2$s cells were found that need to be updated, %3$s changes were made.', 'gi-toolkit' ),
			...
			<div class="gi-toolkit__section search-replace-real-run-section" id="JS-search-replace-real-run-section">
				<div class="gi-toolkit__section__body">
					<div class="gi-toolkit__section__body__header">
						<div class="gi-toolkit__section__body__header__title">
							<?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/replace.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?>
						...
						<div class="gi-toolkit__section__body__header__status" id="JS-search-replace-real-run-status">(<?php esc_html_e( 'Progression', 'gi-toolkit' ); ?>)</div>
					</div>
					<div class="gi-toolkit__section__body__body">
						<div class="gi-toolkit__section__body__body__top">
							<div class="gi-toolkit__section__body__body__data">
								<div class="gi-toolkit__section__body__body__data__percentage" id="JS-search-replace-real-run-percentage">0%</div>
								<div class="gi-toolkit__section__body__body__data__size" id="JS-search-replace-real-run-size">(0 MB / 0 MB)</div>
								<div class="gi-toolkit__section__body__body__data__time" id="JS-search-replace-real-run-time">00:00:00</div>
							</div>
							<div class="gi-toolkit__section__body__body__actions">
								<span class="gi-toolkit__section__body__body__actions__stop" id="JS-search-replace-real-run-stop">
									<span class="gi-toolkit__section__body__body__actions__stop__icon"></span>
									<?php esc_html_e( 'Stop', 'gi-toolkit' ); ?>
								</span>
							</div>
						...
						<div class="gi-toolkit__section__body__body__bottom">
							<div class="gi-toolkit__section__body__body__progress-bar">
								<div class="gi-toolkit__section__body__body__progress-bar__fill" id="JS-search-replace-real-run-progress-bar"></div>
							</div>
						</div>
					</div>
					<div class="gi-toolkit__section__body__footer">
			...
			<div class="gi-toolkit__section search-replace-results-section" id="JS-search-replace-results-section">
				<div class="gi-toolkit__section__body">
					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__title"><?php esc_html_e( "Preview change", 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content">

							<div class="gi-toolkit__section__body__item__content__results" id="JS-search-replace-results"></div>

							<?php if ( ! gi_toolkit_is_pro() ) : ?>
							<div class="gi-toolkit__section__body__item__content__dumy-pro" id="JS-search-replace-dumy-pro">
								<div class="gi-toolkit__tooltip">
									<div class="gi-toolkit__tooltip__text"><?php esc_html_e( 'To view all hidden results, upgrade to the version', 'gi-toolkit' ); ?></div>
								</div>
							</div>
							<div class="gi-toolkit__section__body__item__content__dumy" id="JS-search-replace-dumy">
	...
	private function search_replace_item( $index, $first = false ) {
		$is_pro = gi_toolkit_is_pro();
		?>
		<div class="gi-toolkit__input-text flex search-replace-item" data-index="<?php echo esc_attr( $index ); ?>">

			<div class="full-width">
				<div class="gi-toolkit__section__body__item__title"><?php esc_html_e('Search for', 'gi-toolkit'); ?></div>
				<input type="text" class="JS-search-replace-item-search">
			</div>

			<div class="icon-plus"><?php echo wp_kses( file_get_contents(GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/arrow-right-long.svg'), gi_toolkit_allowed_tags_for_svg_files() ); ?></div>

			<div class="full-width">
				<div class="gi-toolkit__section__body__item__title"><?php esc_html_e('Replace with', 'gi-toolkit'); ?></div>
				<input type="text" class="JS-search-replace-item-replace">
			</div>

			<div class="buttons">
				<label class="gi-toolkit__tooltip__container" title="<?php esc_attr_e( 'Use regular expressions', 'gi-toolkit' ); ?>">
					<input type="checkbox" class="JS-search-replace-item-regex" <?php disabled( $is_pro, false ); ?>>
					<span class="gi-toolkit__tooltip__trigger">.*</span>
				...
				<label title="<?php esc_attr_e( 'Match case', 'gi-toolkit' ); ?>">
			...
			<div class="actions">
				<span class="JS-search-replace-item-remove remove <?php echo esc_attr( $first ? 'first' : '' ); ?>"><?php echo wp_kses( file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/times.svg' ), gi_toolkit_allowed_tags_for_svg_files() ); ?></span>
			</div>
	...
	private function search_replace_replace_item( $table = '{{table}}', $row_id = '{{row_id}}', $column = '{{column}}', $text = '{{text}}' ) {
		?>
			<div class="gi-toolkit__section__body__item__content__results__item">
				<div class="gi-toolkit__section__body__item__content__results__item__top">
					<div class="gi-toolkit__section__body__item__content__results__item__top__table"><?php echo esc_html( $table ); ?></div>
					<div class="gi-toolkit__section__body__item__content__results__item__top__row">
						<div class="gi-toolkit__section__body__item__content__results__item__top__row__label"><?php esc_html_e( 'Row ID:', 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content__results__item__top__row__value"><?php echo esc_html( $row_id ); ?></div>
					</div>
					<div class="gi-toolkit__section__body__item__content__results__item__top__column">
						<div class="gi-toolkit__section__body__item__content__results__item__top__column__label"><?php esc_html_e( 'Column:', 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content__results__item__top__column__value"><?php echo esc_html( $column ); ?></div>
					</div>
				</div>
				<div class="gi-toolkit__section__body__item__content__results__item__bottom"><?php echo wp_kses_post( $text ); ?></div>
			</div>
		<?php
	}
}
