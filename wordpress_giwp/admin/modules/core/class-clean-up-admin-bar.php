 ...
 * @since 1.4.0
 */
class Gi_Toolkit_Clean_Up_Admin_Bar {
    ...
    public function __construct() {
        $this->option_id    = GI_TOOLKIT_PLUGIN_SETTINGS . '_clean_up_admin_bar';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Clean Up Admin Bar', 'gi-toolkit' );
    }
    ...
    public function clean_up_admin_bar( $wp_admin_bar ) {
        ...
        if ( isset( $elements['howdy'] ) && $elements['howdy'] == '1' ) {
             ...
             */
            $title = apply_filters( 'gi_toolkit/clean_up_admin_bar/logout_label', '⏻' );
    ...
    public function add_submenu(){
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-clean-up-admin-bar',
    ...
    public function render_submenu() {
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/clean-up-admin-bar.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/clean-up-admin-bar.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/clean-up-admin-bar.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e( "Remove various elements from the admin bar.", 'gi-toolkit'); ?></div>
    ...
    private function get_elements() {
        $result = array(
            'logo_menu'        => __( 'Remove WordPress logo', 'gi-toolkit' ),
            'customize_menu'   => __( 'Remove customize menu', 'gi-toolkit' ),
            'updates_counter'  => __( 'Remove updates counter', 'gi-toolkit' ),
            'comments_counter' => __( 'Remove comments counter', 'gi-toolkit' ),
            'new_content_menu' => __( 'Remove new content menu', 'gi-toolkit' ),
            'howdy'            => __( 'Remove \'Howdy\'', 'gi-toolkit' ),
            'help_drawer'      => __( 'Remove the Help drawer', 'gi-toolkit' ),
        );
