 ...
 * @since 1.0.0
 */
class Gi_Toolkit_Lock_Site_URL {
    ...
    public static function activate(){
        ...
        if( !defined('WP_HOME') )    Gi_Toolkit_WP_Config::replace_or_add_constant('WP_HOME', get_option('home'), 'string' );
        if( !defined('WP_SITEURL') ) Gi_Toolkit_WP_Config::replace_or_add_constant('WP_SITEURL', get_option('siteurl'), 'string' );
        if( !defined('RELOCATE') )   Gi_Toolkit_WP_Config::replace_or_add_constant('RELOCATE', false );
    }
    ...
    public static function deactivate(){
        require_once GI_TOOLKIT_PLUGIN_PATH . 'admin/class-wp-config.php';
        Gi_Toolkit_WP_Config::remove_constant('WP_HOME');
        Gi_Toolkit_WP_Config::remove_constant('WP_SITEURL');
        Gi_Toolkit_WP_Config::remove_constant('RELOCATE');
    }
    ...
    public function disable_users_can_register() {
        ...
        wp_localize_script( 'Gi_Toolkit_wp-options-general', 'gi_toolkit_lock_site_url', array(
            'i18n' => array(
                'disable_site_url' => esc_js( esc_html__( '🔒 Locked for better security', 'gi-toolkit' ) ),
            ),
        ) );
    }
