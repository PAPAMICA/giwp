 ...
 * @since 1.4.0
 */
class Gi_Toolkit_Enhance_List_Tables {
    ...
    public function __construct() {
        $this->option_id    = GI_TOOLKIT_PLUGIN_SETTINGS . '_enhance_list_tables';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Enhance List Tables', 'gi-toolkit' );
    }
    ...
    public function add_post_type_column( $post_columns, $post_type = 'page' ) {
        ...
        if ( isset( $for_post_types['id_column'] ) && $for_post_types['id_column'] == '1' ) {
            $post_columns['gi_toolkit-id'] = __( 'ID', 'gi-toolkit' );
        }   
        ...
        if ( isset( $for_post_types['image_column'] ) && $for_post_types['image_column'] == '1' && $supports_thumbnail ) {
            $post_columns['gi_toolkit-image'] = __( 'Featured image', 'gi-toolkit' );
        }
        ...
        if ( isset( $for_post_types['excerpt_column'] ) && $for_post_types['excerpt_column'] == '1' && $supports_excerpt ) {
            $post_columns['gi_toolkit-excerpt'] = __( 'Excerpt', 'gi-toolkit' );
        }
    ...
    public function render_post_type_column( $column_name, $post_id ) {
        if ( 'gi_toolkit-id' === $column_name ) {
            echo esc_html( $post_id );
        }
        if ( 'gi_toolkit-image' === $column_name ) {
            echo get_the_post_thumbnail( $post_id, array( 70, 70 ) );
        }
        if ( 'gi_toolkit-excerpt' === $column_name ) {
    ...
    public function add_taxonomy_column( $columns ) {
        ...
        if ( isset( $for_taxonomies['id_column'] ) && $for_taxonomies['id_column'] == '1' ) {
            $columns['gi_toolkit-id'] = __( 'ID', 'gi-toolkit' );
        }
    ...
    public function add_media_column( $columns ) {
        ...
        if ( isset( $for_media['file_size_column'] ) && $for_media['file_size_column'] == '1' ) {
            $columns['gi_toolkit-file-size'] = __( 'File Size', 'gi-toolkit' );
        }
    ...
    public function render_media_column( $column_name, $post_id ) {
        ...
        if ( 'gi_toolkit-file-size' === $column_name ) {
        ...
        if ( property_exists( $object, 'ID' ) ) {
            ...
            if ( is_a( $object, 'WP_User' ) ) {
                if ( isset( $settings['for_users']['id_action'] ) && $settings['for_users']['id_action'] == '1' ) {
                    $actions['gi_toolkit-id'] = 'ID: ' . $object->ID;
                }
    ...
    public function add_submenu(){
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-enhance-list-tables',
    ...
    public function render_submenu() {
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/content-order.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/content-order.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/content-order.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e( "Improve the usefulness of listing pages for various post types and taxonomies, media, comments and users by adding / removing columns and elements.", 'gi-toolkit'); ?></div>
                <div class="gi-toolkit__section__body">

                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Post types', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content flex flex-wrap">
                            <?php foreach ( $for_post_types as $element_id => $element_label ): ?>
                                <div class="gi-toolkit__checkbox">
                                    <label class="gi-toolkit__checkbox__label">
                                        ...
                                        <span class="gi-toolkit__checkbox__label__text"><?php echo esc_html( $element_label ); ?></span>
                                    </label>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Taxonomies', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content flex flex-wrap">
                    ...
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Media', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content flex flex-wrap">
                    ...
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Comments', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content flex flex-wrap">
                    ...
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Users', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content flex flex-wrap">
    ...
    private function get_elements_for_post_types() {
        $result = array(
            'title_column'      => __( 'Remove title column', 'gi-toolkit' ),
            'author_column'     => __( 'Remove author column', 'gi-toolkit' ),
            'categories_column' => __( 'Remove categories column', 'gi-toolkit' ),
            'tags_column'       => __( 'Remove tags column', 'gi-toolkit' ),
            'comments_column'   => __( 'Remove comments column', 'gi-toolkit' ),
            'date_column'       => __( 'Remove date column', 'gi-toolkit' ),
            'id_column'         => __( 'Add ID column', 'gi-toolkit' ),
            'id_action'         => __( 'Add ID in actions.', 'gi-toolkit' ),
            'image_column'      => __( 'Add featured image column', 'gi-toolkit' ),
            'excerpt_column'    => __( 'Add excerpt column', 'gi-toolkit' ),
        );
    ...
    private function get_elements_for_taxonomies() {
        $result = array(
            'name_column'        => __( 'Remove name column', 'gi-toolkit' ),
            'description_column' => __( 'Remove description column', 'gi-toolkit' ),
            'slug_column'        => __( 'Remove slug column', 'gi-toolkit' ),
            'posts_column'       => __( 'Remove count column', 'gi-toolkit' ),
            'id_column'          => __( 'Add ID column', 'gi-toolkit' ),
            'id_action'          => __( 'Add ID in actions.', 'gi-toolkit' ),
        );
    ...
    private function get_elements_for_media() {
        $result = array(
            'title_column'     => __( 'Remove File column', 'gi-toolkit' ),
            'author_column'    => __( 'Remove author column', 'gi-toolkit' ),
            'parent_column'    => __( 'Remove uploaded to column', 'gi-toolkit' ),
            'comments_column'  => __( 'Remove comments column', 'gi-toolkit' ),
            'date_column'      => __( 'Remove date column', 'gi-toolkit' ),
            'id_column'        => __( 'Add ID column', 'gi-toolkit' ),
            'id_action'        => __( 'Add ID in actions.', 'gi-toolkit' ),
            'file_size_column' => __( 'Add file size column', 'gi-toolkit' ),
        );
    ...
    private function get_elements_for_comments() {
        $result = array(
            'author_column'   => __( 'Remove author column', 'gi-toolkit' ),
            'comment_column'  => __( 'Remove comment column', 'gi-toolkit' ),
            'response_column' => __( 'Remove in response to column', 'gi-toolkit' ),
            'date_column'     => __( 'Remove submitted on column', 'gi-toolkit' ),
            'id_column'       => __( 'Add ID column', 'gi-toolkit' ),
            'id_action'       => __( 'Add ID in actions.', 'gi-toolkit' ),
        );
    ...
    private function get_elements_for_users() {
        $result = array(
            'username_column' => __( 'Remove username column', 'gi-toolkit' ),
            'name_column'     => __( 'Remove name column', 'gi-toolkit' ),
            'email_column'    => __( 'Remove email column', 'gi-toolkit' ),
            'email_column'    => __( 'Remove email column', 'gi-toolkit' ),
            'role_column'     => __( 'Remove role column', 'gi-toolkit' ),
            'posts_column'    => __( 'Remove posts column', 'gi-toolkit' ),
            'id_column'       => __( 'Add ID column', 'gi-toolkit' ),
            'id_action'       => __( 'Add ID in actions.', 'gi-toolkit' ),
        );
