 ...
 * @since 1.10.0
 */
class Gi_Toolkit_Plugin_Theme_Rollback {
    ...
    public function __construct() {
		$this->option_id      = GI_TOOLKIT_PLUGIN_SETTINGS . '_plugin_theme_rollback';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Plugin & Theme Rollback', 'gi-toolkit' );
    }
	...
	public function enqueue_admin_scripts_styles( $hook_suffix ) {
		if ( 'themes.php' === $hook_suffix ) {
			$themes_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/plugin-theme-rollback-themes.asset.php' );
			wp_enqueue_script( 'Gi_Toolkit_plugin_theme_rollback_themes', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/plugin-theme-rollback-themes.js', $themes_assets['dependencies'], $themes_assets['version'], true );
			wp_localize_script( 'Gi_Toolkit_plugin_theme_rollback_themes', 'Gi_ToolkitPluginThemeRollbackThemes', array(
				'ajaxUrl'  => admin_url(),
				'nonce'    => wp_create_nonce( $this->user_nonce ),
				'optionId' => $this->option_id,
				'i10n'     => array(
					'rollbackLabel'   => __( 'Rollback', 'gi-toolkit' ),
					'notRollbackable' => __( 'No Rollback Available: This is a non-WordPress.org theme.', 'gi-toolkit' ),
					'loadingRollback' => __( 'Loading...', 'gi-toolkit' ),
				),
			));
		}
	}
	...
	public function add_plugin_action_links( $actions, $plugin_file, $plugin_data ) {
		...
		$actions[$this->option_id] = '<a href="' . esc_url( $rollback_url ) . '">' . __( 'Rollback', 'gi-toolkit' ) . '</a>';
	...
	public function theme_updates_list() {
		...
		if ( $ssl && is_wp_error( $raw_response ) ) {
			...
			trigger_error( __( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'gi-toolkit' ) . ' ' . __( '(WordPress could not establish a secure connection to WordPress.org. Please contact your server administrator.)', 'gi-toolkit' ), headers_sent() || WP_DEBUG ? E_USER_WARNING : E_USER_NOTICE );
    ...
    public function render_submenu() {
        ...
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/plugin-theme-rollback.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/plugin-theme-rollback.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/plugin-theme-rollback.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
	...
	private function set_plugin_slug() {
		...
		if ( ! file_exists( $plugin_file ) ) {
			wp_die( esc_html__( 'Plugin you\'re referencing does not exist.', 'gi-toolkit' ) );
		}
	...
	private function set_svn_versions_data( $html ) {
		...
		if ( ( $json = json_decode( $html ) ) && ( $html != $json ) ) {
			$gi_toolkit_versions = array_keys( (array) $json->versions );
		} else {
			$obj = new DOMDocument();
			$obj->loadHTML( $html );
			$gi_toolkit_versions = array();
			...
			foreach ( $items as $item ) {
				...
				if ( strpos( $href, 'http' ) === false && '..' !== $href ) {
					$gi_toolkit_versions[] = $href;
				}
			}
		}

		$gi_toolkit_versions = array_reverse( $gi_toolkit_versions );
		return $gi_toolkit_versions;
	}
    ...
    private function submenu_content() {
		...
		if ( ! empty( $args['plugin_version'] ) ) {
			if ( empty( $args['plugin_file'] ) || ! file_exists( WP_PLUGIN_DIR . '/' . $args['plugin_file'] ) ) {
				$error_msg = __( 'Necessary parameters are missing. Please try again.', 'gi-toolkit' );
			} else {
				include GI_TOOLKIT_PLUGIN_PATH . 'admin/helpers/core/plugin-theme-rollback/class-rollback-plugin.php';
				...
				$upgrader = new Gi_Toolkit_Plugin_Theme_Rollback_Plugin( new Plugin_Upgrader_Skin( compact( 'title', 'nonce', 'url', 'plugin', 'version' ) ) );
				$upgrader->gi_toolkit_rollback_module( plugin_basename( sanitize_text_field( $args['plugin_file'] ) ) );
			}
		} elseif ( ! empty( $args['theme_version'] ) ) {
			if ( empty( $args['theme_file'] ) || ! file_exists( WP_CONTENT_DIR . '/themes/' . $args['theme_file'] ) ) {
				$error_msg = __( 'Necessary parameters are missing. Please try again.', 'gi-toolkit' );
			} else {
				include GI_TOOLKIT_PLUGIN_PATH . 'admin/helpers/core/plugin-theme-rollback/class-rollback-theme.php';
				...
				$upgrader = new Gi_Toolkit_Plugin_Theme_Rollback_Theme( new Theme_Upgrader_Skin( compact( 'title', 'nonce', 'url', 'theme', 'version' ) ) );
				$upgrader->gi_toolkit_rollback_module( $args['theme_file'] );
			}
		} else {
		...
		if ( ! empty( $error_msg ) ) {
			?>
				<div class="gi-toolkit__section">
					<div class="gi-toolkit__section__desc"><?php echo esc_html( $error_msg ); ?></div>
				</div>
			<?php
		}
    }
	...
	private function show_menu( $args ) {
		...
		if ( isset( $args['plugin_file'] ) && in_array( $args['plugin_file'], array_keys( $plugins ) ) ) {
			$gi_toolkit_versions = $this->versions_select( 'plugin', $args );
			//phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$gi_toolkit_versions = $this->versions_select( 'theme', $args );
		}

		?>
			<div class="gi-toolkit__section">
				<div class="gi-toolkit__section__desc"></div>
					...
					<form name="check_for_rollbacks" class="rollback-form" action="<?php echo esc_url( admin_url( '/admin.php' ) ); ?>">
						...
						<div class="gi-toolkit__section__body__item">
							<div class="gi-toolkit__section__body__item__title">
								<?php
								if ( $theme_rollback ) {
									esc_html_e( 'Theme Name', 'gi-toolkit' );
								} elseif ( $plugin_rollback ) {
									esc_html_e( 'Plugin Name', 'gi-toolkit' );
								}
								?>
							</div>
						...
						<div class="gi-toolkit__section__body__item">
							<div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Current Version', 'gi-toolkit' ); ?></div>
							<div class="gi-toolkit__section__body__item__content">
								<div class="description">
						...
						<div class="gi-toolkit__section__body__item">
							<div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'New Version', 'gi-toolkit' ); ?></div>
							<div class="gi-toolkit__section__body__item__content">
								<?php
								//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								echo $gi_toolkit_versions;
								?>
							</div>
						</div>
						...
						<div class="gi-toolkit__section__body__item">
							<div class="gi-toolkit__section__body__item__content">
								<div class="gi-toolkit__button">
									<button type="submit"><?php esc_html_e( 'Rollback', 'gi-toolkit' ); ?></button>
								</div>
								<div class="description"><?php  esc_html_e( 'We strongly recommend you create a complete backup, before you take this action.', 'gi-toolkit' ); ?></div>
	...
	private function versions_select( $type, $args ) {
		...
		if ( empty( $gi_toolkit_versions ) ) {
			return '<div class="description">' . sprintf(
				/* translators: %s: type */
				__( 'It appears there are no version to select. This is likely due to the %s author not using tags for their versions and only committing new releases to the repository trunk.', 'gi-toolkit' ), 
				$type 
			) . '</div>';
		}

		usort( $gi_toolkit_versions, 'version_compare' );
		$gi_toolkit_versions = array_reverse( $gi_toolkit_versions );

		$versions_html = '<div class="gi-toolkit__select">';
		$versions_html .= '<select name="' . esc_attr( $type . '_version' ) .'">';
		foreach ( $gi_toolkit_versions as $version ) {
		...
		$versions_html .= '<div class="description">' . sprintf( 
			/* translators: %s: type */
			__( 'Select the %s version you want to rollback to.', 'gi-toolkit' ), 
			$type 
		) . '</div>';
