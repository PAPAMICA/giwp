 ...
 * @since 1.3.0
 */
class Gi_Toolkit_Export_Users {

    const ACTION = 'gi_toolkit_export_users';
    const NONCE  = 'gi_toolkit_export_users_nonce';
    ...
    public function add_export_action( $actions, $user_object ) {
        ...
        $actions[ self::ACTION ] = sprintf( '<a href="%s" target="_blank">%s</a>', esc_url( $url ), esc_html__( 'Download csv', 'gi-toolkit' ) );
    ...
    public function add_bulk_action( $actions ) {
        ...
        if ( ! isset( $actions[ self::ACTION ] ) ) {
            ...
            $actions[ self::ACTION ] = __( 'Download csv', 'gi-toolkit' );
        }
    ...
    public function download_user() {
        ...
        if ( ! current_user_can( 'export' ) && ! current_user_can( 'list_users' ) ) {
            wp_die( esc_html__( 'Droits insuffisants pour exporter des utilisateurs.', 'gi-toolkit' ) );
        }
