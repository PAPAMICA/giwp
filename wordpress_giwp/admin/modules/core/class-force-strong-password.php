 ...
 * @since 1.0.0
 */
class Gi_Toolkit_Force_Strong_Password {
    /**
    ...
    public function custom_password_strength_meter_localization(){
        ...
        wp_localize_script( 'password-strength-meter', 'pwsL10n', array(
            'unknown'  => esc_attr_x( 'Password strength unknown', 'password strength', 'gi-toolkit' ),
            'short'    => esc_attr_x( "Not authorized", 'password strength', 'gi-toolkit' ),
            'bad'      => esc_attr_x( 'Not authorized',  'password strength', 'gi-toolkit' ),
            'good'     => esc_attr_x( 'Medium', 'password strength', 'gi-toolkit' ),
            'strong'   => esc_attr_x( 'Strong', 'password strength', 'gi-toolkit' ),
            'mismatch' => esc_attr_x( 'Mismatch', 'password mismatch', 'gi-toolkit' ),
        ) );
    }
    ...
    private function validate_complex_password( $errors ) {
        ...
        if ( ! $this->is_strong_password( $password ) ) {
            $errors->add( 
                'pass', 
                wp_kses_post( '<strong>ERROR</strong>: Your password is not strong enough. Please use a stronger password.', 'gi-toolkit' ),
