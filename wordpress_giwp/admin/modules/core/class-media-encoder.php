 ...
 * @since 1.13.0
 */
class Gi_Toolkit_Media_Encoder {
    ...
    public function __construct() {
        $this->option_id   	                    = GI_TOOLKIT_PLUGIN_SETTINGS . '_media_encoder';
		...
		add_action( 'wp_ajax_gi_toolkit_media_encoder_preview_mode', array( $this, 'preview_mode_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_start_bulk', array( $this, 'start_bulk_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_stop_bulk', array( $this, 'stop_bulk_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_progress_bulk', array( $this, 'progress_bulk_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_start_single', array( $this, 'start_single_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_undo_single', array( $this, 'undo_single_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_start_single_migration', array( $this, 'start_single_migration_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_start_bulk_migration', array( $this, 'start_bulk_migration_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_stop_bulk_migration', array( $this, 'stop_bulk_migration_cb' ) );
		add_action( 'wp_ajax_gi_toolkit_media_encoder_progress_bulk_migration', array( $this, 'progress_bulk_migration_cb' ) );

		add_action( 'template_redirect', array( $this, 'start_content_process' ), -1000 );
    }

	/**
	 * Vérifie nonce et droits pour les requêtes AJAX du module.
	 *
	 * @return void
	 */
	private function verify_ajax_request() {
		gi_toolkit_ajax_require_cap( 'upload_files', $this->nonce_action );
	}
    ...
    public function class_init() {
		$this->header_title	= esc_html__( 'Media Encoder', 'gi-toolkit' );
    ...
    public function add_submenu(){
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-media-encoder',
	...
	public function add_already_optimized_meta( $metadata, $attachment_id, $context ) {
		...
		$gi_toolkit = sanitize_text_field( wp_unslash( $_FILES['async-upload']['gi_toolkit'] ?? '' ) );
		
		if ( 'optimized' == $gi_toolkit ) {
			//phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function add_attachment_fields_to_edit( $form_fields, $post ) {
		...
		switch ( $status ) {
			...
			break;
			case '2':
				$html = '<br>' . esc_html__( 'Image already optimized.', 'gi-toolkit' );
			break;
		}
		
		$form_fields['gi_toolkit_media_encoder'] = array(
			'label' 		=> 'Gi_Toolkit',
			'input' 		=> 'html',
			'html' 			=> $html,
			'show_in_edit'	=> true,
	...
	public function add_media_columns( $columns ) {
		...
		$columns['gi-toolkit'] = __( 'GI-Toolkit', 'gi-toolkit' );
	...
	public function add_media_custom_column( $column_name, $attachment_id ) {
		...
		if ( 'gi-toolkit' !== $column_name ) {
			return;
		}

		if ( ! $this->valid_mimetype( $attachment_id ) ) {
			esc_html_e( 'Image type not supported', 'gi-toolkit' );
			return;
		}
		...
		switch ( $status ) {
			...
			break;
			case '2':
				echo wp_kses_post( '<br>' . esc_html__( 'Image already optimized.', 'gi-toolkit' ) );
			break;
		}
	}
	...
	public function add_attachment_submitbox_misc_actions() {
		...
		echo '<table><tr><td><div><strong>GI-Toolkit</strong></div>';
		switch ( $status ) {
			case '0':
			...
			break;
			case '2':
				echo esc_html__( 'Image already optimized.', 'gi-toolkit' );
			break;
		}
		echo '</td></tr></table>';
	}
	...
	public function enqueue_scripts_styles( $suffix ) {
		...
		if ( 'upload.php' == $suffix || ( 'post.php' == $suffix && 'attachment' == $post_type ) ) {
			$submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/media-encoder-wpmedia.asset.php' );
			wp_enqueue_style( 'Gi_Toolkit_wpmedia', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/media-encoder-wpmedia.css', array(), $submenu_assets['version'], 'all' );
			wp_enqueue_script( 'Gi_Toolkit_wpmedia', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/media-encoder-wpmedia.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
			wp_localize_script( 'Gi_Toolkit_wpmedia', 'Gi_Toolkit_media_encoder_wpmedia', array(
	...
	public function preview_mode_cb() {
		...
		if ( empty( $file ) ) {
			wp_send_json_error( __( 'No image uploaded, try again.', 'gi-toolkit' ) );
		}
	...
	public function start_bulk_cb() {
		...
		$mode_enabled = $this->settings['mode_enabled']['value'] ?? $this->default_settings['mode_enabled']['value'];
		if ( 'off' === $mode_enabled ) {
			wp_send_json_error( __( 'Choose an image format and save the settings.', 'gi-toolkit' ) );
		}

		$media_ids = $this->get_unoptimized_media_ids();
		if ( empty( $media_ids ) ) {
			wp_send_json_error( __( 'No images to optimize.', 'gi-toolkit' ) );
		}

		$status = get_option( $this->option_bulk_status, 'finish' );
		if ( $status != 'finish' ) {
			wp_send_json_error( __( 'Bulk optimization is already running.', 'gi-toolkit' ) );
		}
		...
		wp_send_json_error( __( 'Refresh the page and try again.', 'gi-toolkit' ) );
	}
	...
	public function stop_bulk_cb() {
		...
		if ( $this->clear_bulk_optimization() ) {
			wp_send_json_success( __( 'Bulk optimization stopped.', 'gi-toolkit' ) );
		}
	...
	public function start_single_cb() {
		...
		if ( ! $attachment_id ) {
			wp_send_json_error( __( 'No attachment id.', 'gi-toolkit' ) );
		}

		$already_optimized = get_post_meta( $attachment_id, $this->meta_already_optimized, true );

		if ( '1' == $already_optimized && 'webp' == $mode_enabled ) {
			wp_send_json_error( __( 'Already optimized.', 'gi-toolkit' ) );
		}
		...
		if ( ! in_array( $post_mime_type, $this->allowed_mime_types ) ) {
			wp_send_json_error( __( 'Not a valid image.', 'gi-toolkit' ) );
		}
	...
	public function undo_single_cb() {
		...
		if ( ! empty( $data ) ) {
			...
			wp_send_json_error( __( 'Not optimized.', 'gi-toolkit' ) );
		}
	}

	/**
	 * Migrate the data from QuickWebp to GI-Toolkit
	 * 
	 * @since   1.13.0
	 */
	...
	public function start_bulk_migration_cb() {
		...
		$media_ids = $this->get_not_migrated_media_ids();
		if ( empty( $media_ids ) ) {
			wp_send_json_error( __( 'No images to migrate.', 'gi-toolkit' ) );
		}

		$status = get_option( $this->option_migrate_status, 'finish' );
		if ( $status != 'finish' ) {
			wp_send_json_error( __( 'Bulk migration is already running.', 'gi-toolkit' ) );
		}
	...
	public function stop_bulk_migration_cb() {
		...
		if ( $this->clear_bulk_migration() ) {
			wp_send_json_success( __( 'Bulk migration stopped.', 'gi-toolkit' ) );
		}
    ...
    public function render_submenu() {
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/media-encoder.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/media-encoder.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/media-encoder.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
		wp_localize_script( 'Gi_Toolkit_submenu', 'Gi_Toolkit_media_encoder', array(
			'ajaxUrl' => admin_url( 'admin-ajax.php' ),
            'nonce'   => wp_create_nonce( $this->nonce_action ),
		));
	...
	private function add_to_htaccess() {
        ...
        if ( $is_apache ) {
			...
			if ( $display_mode == 'rewrite' ) {
				...
				Gi_Toolkit_Htaccess::add( $this->get_raw_content_htaccess(), self::MODULE_ID );
			}
        }
	}
	...
	private function process_image( $image ) {
        ...
        if ( ! empty( $attributes['class'] ) && strpos( $attributes['class'], 'gi_toolkit-no-webp' ) !== false ) {
			// Has the 'gi_toolkit-no-webp' class.
			return false;
		}
	...
	private function optimize_btn( $attachment_id ) {
		...
		$class = 'gi_toolkit-single-optimization-btn';
		$text  = __( 'Optimize', 'gi-toolkit' );

		if ( $optimized_with_quickwebp ) {
			$class = 'gi_toolkit-single-migrate-btn';
			$text  = __( 'Migrate from quickWebP', 'gi-toolkit' );
		}
		...
		ob_start();
			?>
				...
				<div class="gi_toolkit-single-optimization-msg"></div>
			<?php
		return ob_get_clean();
	}
	...
	private function attachment_data( $data, $attachment_id ) {
		...
		if ( ! empty( $full_image_data ) ) {
			ob_start();
				?>
					<div>
						<strong><?php esc_html_e( 'Original Image: ', 'gi-toolkit' ); ?></strong>
					...
					<div>
						<strong><?php esc_html_e( 'Save: ', 'gi-toolkit' ); ?></strong>
						<span><?php echo esc_html( $full_image_data['percent'] . '%' ); ?></span>
					</div>
					<div>
						<button type="button" class="button button-sacondary gi_toolkit-undo-single-optimization-btn" data-attachment-id="<?php echo esc_attr( $attachment_id ); ?>">
							<?php esc_html_e( 'Undo optimization', 'gi-toolkit' ); ?>
							<div class="spinner"></div>
						</button>
						<div class="gi_toolkit-undo-single-optimization-msg"></div>
					</div>
				<?php
			return ob_get_clean();
		}
    ...
    private function get_default_settings(){
        ...
        return array(
			'mode_enabled'       => array(
				'value'   => 'webp',
				'options' => array(
					'off'  => __( 'Off', 'gi-toolkit' ),
					'webp' => __( 'WebP', 'gi-toolkit' ),
					'avif' => __( 'AVIF', 'gi-toolkit' ),
				)
			),
			'quality'            => array(
				'value'   => 'medium',
				'options' => array(
					'low'    => __( 'Low', 'gi-toolkit' ),
					'medium' => __( 'Medium', 'gi-toolkit' ),
					'high'   => __( 'High', 'gi-toolkit' ),
				)
			),
			...
			'display_mode'       => array(
				'value'   => 'disabled',
				'options' => array(
					'disabled' => __( 'Deactivate', 'gi-toolkit' ),
					'picture'  => __( 'Use <picture> tags', 'gi-toolkit' ),
					'rewrite'  => __( 'Use rewrite rules', 'gi-toolkit' ),
				),
			),
        );
    }
		...
		if ( $this->quickwebp_plugin_activated ) {
			?>
				<div class="gi-toolkit__section">
					<div class="gi-toolkit__section__notice show">
						<p class="gi-toolkit__section__notice__message">
							<?php 
							echo wp_kses_post( sprintf( 
								/* translators: %s: plugin name */
								__( "Please deactivate %s plugin.", 'gi-toolkit' ), 
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc">
					<?php esc_html_e( "Automatically converts uploaded images to your selected format (WebP or AVIF) for better performance and reduced file size.", 'gi-toolkit'); ?>
				</div>
                <div class="gi-toolkit__section__body">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title">
							<?php esc_html_e( 'Image Format Optimization', 'gi-toolkit' ); ?>
							<div class="gi-toolkit__section__body__item__title__info">
								<div class="gi-toolkit__section__body__item__title__info__icon">
									<?php echo wp_kses( file_get_contents(GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/info.svg'), gi_toolkit_allowed_tags_for_svg_files() ); ?>
								</div>
								<div class="gi-toolkit__section__body__item__title__info__popup">
									<p><?php esc_html_e( 'Select “Off” to disable image optimization, “WebP” for wide browser support, or “AVIF” for best compression and smaller file sizes.', 'gi-toolkit' ); ?></p>
								</div>
							</div>
						</div>
                                ...
                                <select name="<?php echo esc_attr( $this->option_id . '[mode_enabled][value]' ); ?>">
                                    <?php
										foreach ( $mode_enabled_options as $key => $name ) {
											...
											$disable_message = '';
											if ( $key == 'avif' && ( ! $php_compatible || ! $imageavif_supported ) ) {
												$option_disabled = true;
												$disable_message = '';
												if ( ! $imageavif_supported ) {
													$disable_message = __( '(imageavif() not supported)', 'gi-toolkit' );
												}
												if ( ! $php_compatible ) {
													$disable_message = __( '(PHP 8.1 or higher)', 'gi-toolkit' );
												}
											}
											?>
					...
					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__title">
							<?php esc_html_e( 'Quality', 'gi-toolkit' ); ?>
							<div class="gi-toolkit__section__body__item__title__info">
								<div class="gi-toolkit__section__body__item__title__info__icon">
									<?php echo wp_kses( file_get_contents(GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/info.svg'), gi_toolkit_allowed_tags_for_svg_files() ); ?>
								</div>
								<div class="gi-toolkit__section__body__item__title__info__popup">
									<p><?php esc_html_e( 'Choose the image quality level. Lower quality results in smaller file sizes, while higher quality provides better visuals but larger files.', 'gi-toolkit' ); ?></p>
								</div>
							</div>
						</div>
                        <div class="gi-toolkit__section__body__item__content">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Do not compress images already in same format', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Save original images', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">
							<div class="gi-toolkit__checkbox">
								<label class="gi-toolkit__checkbox__label">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title">
							<?php esc_html_e( 'Display images with new format on the site', 'gi-toolkit' ); ?>
							<div class="gi-toolkit__section__body__item__title__info">
								<div class="gi-toolkit__section__body__item__title__info__icon">
									<?php echo wp_kses( file_get_contents(GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/info.svg'), gi_toolkit_allowed_tags_for_svg_files() ); ?>
								</div>
								<div class="gi-toolkit__section__body__item__title__info__popup">
									<p><?php esc_html_e( 'This option allows to override the original images by the new version (useless for images converted in import)', 'gi-toolkit' ); ?></p>
								</div>
							</div>
						</div>
                        <div class="gi-toolkit__section__body__item__content">
					...
					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Before saving, test your configuration with preview mode', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">
							<div class="gi-toolkit__button">
								<button type="button" class="gi-toolkit__button__open-preview secondary"><?php esc_html_e( 'Preview', 'gi-toolkit' ); ?></button>
							</div>
                        </div>
                    </div>

					<div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Bulk optimization', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">

							<div class="gi-toolkit__section__bulk">
								<div class="gi-toolkit__section__bulk__top">
									<div class="gi-toolkit__button">
										<button type="button" class="secondary start <?php echo $is_running ? '' : 'show'; ?>">
											<?php esc_html_e( 'Start', 'gi-toolkit' ); ?>
									...
									<div class="gi-toolkit__button">
										<button type="button" class="danger stop <?php echo $is_running ? 'show' : ''; ?>">
											<?php esc_html_e( 'Stop', 'gi-toolkit' ); ?>
										</button>
									</div>
								</div>

								<div class="gi-toolkit__section__bulk__bottom">
									<div class="gi-toolkit__section__bulk__progress <?php echo $is_running ? 'show' : ''; ?>">
										<div class="gi-toolkit__section__bulk__progress__inner" style="width:<?php echo esc_attr( $percent ); ?>;"></div>
										<span class="gi-toolkit__section__bulk__progress__progress"><?php echo esc_html( $progress ); ?></span>
									</div>

									<div class="gi-toolkit__section__bulk__message description"></div>
								</div>
							</div>
					...
					<div class="gi-toolkit__section__body__item">
						<div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Bulk migration from QuickWebP', 'gi-toolkit' ); ?></div>
						<div class="gi-toolkit__section__body__item__content">

							<div class="gi-toolkit__section__migrate">
								<div class="gi-toolkit__section__migrate__top">
								...
								<div class="gi-toolkit__section__migrate__bottom">
									<div class="gi-toolkit__section__migrate__progress <?php echo $is_running_migration ? 'show' : ''; ?>">
										<div class="gi-toolkit__section__migrate__progress__inner" style="width:<?php echo esc_attr( $percent_migration ); ?>;"></div>
										<span class="gi-toolkit__section__migrate__progress__progress"><?php echo esc_html( $progress_migration ); ?></span>
									</div>

									<div class="gi-toolkit__section__migrate__message description"></div>
								</div>
							</div>
				...
				<div class="gi-toolkit__section__preview">
					<div class="gi-toolkit__section__preview__file show">
						<div class="gi-toolkit__section__preview__file__btn">
							<?php echo file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/add-img.svg' );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							<span><?php esc_html_e( 'Add image', 'gi-toolkit' ); ?></span>
						</div>
						<input type="file" class="gi-toolkit__section__preview__file__input" accept='image/*'>
					</div>

					<div class="gi-toolkit__section__preview__compare">
						<div class="gi-toolkit__section__preview__compare__images">
							<div class="gi-toolkit__section__preview__compare__images__original">
								<div class="image"></div>
							</div>
							<div class="gi-toolkit__section__preview__compare__images__new">
								<div class="image"></div>
							</div>
						</div>

						<div class="gi-toolkit__section__preview__compare__handle">
							<div class="gi-toolkit__section__preview__compare__handle__svg">
								<?php echo file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/resize.svg' );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</div>
						</div>

						<div class="gi-toolkit__section__preview__compare__data">

							<div class="gi-toolkit__section__preview__compare__data__original">
								<div class="gi-toolkit__section__preview__compare__data__original__type"><?php esc_html_e( 'Original Image', 'gi-toolkit' ); ?></div>
								<div class="gi-toolkit__section__preview__compare__data__original__size"></div>
							</div>

							<div class="gi-toolkit__section__preview__compare__data__new">
								<div class="gi-toolkit__section__preview__compare__data__new__type"></div>
								<div class="gi-toolkit__section__preview__compare__data__new__size"></div>
								<div class="gi-toolkit__section__preview__compare__data__new__gain"></div>
							</div>

						</div>
					</div>

					<div class="gi-toolkit__section__preview__spiner">
						<div class="gi-toolkit__section__preview__spiner__circle"></div>
					</div>

					<div class="gi-toolkit__section__preview__close">
            			<button type="button" class="gi-toolkit__section__preview__close__btn"><?php echo file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/times.svg' );//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></button>
        			</div>
				</div>
            </div>
        <?php
