 ...
 * @since 1.3.0
 */
class Gi_Toolkit_Password_Protection {

    const COOKIE_ID             = 'gi_toolkit_password_protection';
    const COOKIE_PASSWORD       = 'MOeldTVhGnL18VfbDtXM7znSYXIUQn3z';
    const LOGIN_HEAD_ACTION     = 'gi_toolkit_password_protection_login_head';
    const ERROR_MESSAGES_ACTION = 'gi_toolkit_password_protection_error_messages';
    ...
    public function __construct() {
        ...
        $this->option_id        = GI_TOOLKIT_PLUGIN_SETTINGS . '_password_protection';
    ...
    public function class_init() {
        $this->header_title = esc_html__( 'Password Protection', 'gi-toolkit' );
    }
    ...
    public function remind_admin() {
        ...
        if ( is_user_logged_in() && current_user_can( 'manage_options' ) ) {
            $wp_admin_bar->add_menu( array(
                'id'    => 'gi_toolkit_password_protection',
                'title' => '',
                'href'  => admin_url( 'admin.php?page=gi-toolkit-settings-password-protection' ),
                'meta'  => array(
                    'title' => __( 'Password protection is currently enabled for this site.', 'gi-toolkit' ),
                ),
            ));
        }
    }
    ...
    public function remind_admin_style() {
        ...
        if ( is_user_logged_in() && current_user_can( 'manage_options' ) ) {
            ?>
                <style>
                    #wpadminbar ul li#wp-admin-bar-gi_toolkit_password_protection {
                        background-color: #c32121;
                    }
                    #wpadminbar ul li#wp-admin-bar-gi_toolkit_password_protection > .ab-item:before {
                        content: "\f160";
                        top: 2px;
                        color: #fff;
                        margin-right: 0px;
                    }
                    #wpadminbar ul li#wp-admin-bar-gi_toolkit_password_protection:hover > .ab-item {
    ...
    public function show_login_form() {
        ...
        if ( isset( $_REQUEST['protected-page'] ) && 'view' == $_REQUEST['protected-page'] ) {
            $password_protection_template = GI_TOOLKIT_PLUGIN_PATH . 'admin/templates/core/password-protection/password-protection.php';
    ...
    public function process_login() {
        global $gi_toolkit_password_protection_errors;
        $gi_toolkit_password_protection_errors = new WP_Error();
        ...
        if ( isset( $_REQUEST['protected_page_pwd'] ) ) {
            ...
            if ( ! empty( $password_input ) ) {
                ...
                if ( $password_input == $stored_password ) {
                    ...
                    $gi_toolkit_password_protection_errors->add( 'incorrect_password', __( 'Incorrect password.', 'gi-toolkit' ) );
                }

            } else {
                $gi_toolkit_password_protection_errors->add( 'empty_password', __( 'Password can not be empty.', 'gi-toolkit' ) );
            }
        }
    }
    ...
    public function add_login_error_messages() {
        ...
        if ( $gi_toolkit_password_protection_errors->get_error_code() ) {

            $messages = '';
            $errors   = '';

            foreach ( $gi_toolkit_password_protection_errors->get_error_codes() as $code ) {

                $severity = $gi_toolkit_password_protection_errors->get_error_data( $code );
                foreach ( $gi_toolkit_password_protection_errors->get_error_messages( $code ) as $error ) {
                    if ( 'message' == $severity ) {
                        $messages .= $error . '<br />';
                    } else {
                        $errors .= $error . '<br />';
    ...
    public function add_submenu(){
        ...
        Gi_Toolkit_Settings::add_submenu_page(
            ...
            $this->header_title,
            'manage_options',
            'gi-toolkit-settings-password-protection', 
    ...
    public function render_submenu() {
        ...
        $submenu_assets = include( GI_TOOLKIT_PLUGIN_PATH . 'admin/assets/build/core/password-protection.asset.php' );
        wp_enqueue_style( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/password-protection.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'Gi_Toolkit_submenu', GI_TOOLKIT_PLUGIN_URL . 'admin/assets/build/core/password-protection.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
    ...
    private function submenu_content() {
        ...
        ?>
            <div class="gi-toolkit__section">
                <div class="gi-toolkit__section__desc"><?php esc_html_e('Password-protect the entire site to hide the content from public view and search engine bots / crawlers. Logged-in administrators can still access the site as usual.', 'gi-toolkit'); ?></div>
                <div class="gi-toolkit__section__body">
                    <div class="gi-toolkit__section__body__item">
                        <div class="gi-toolkit__section__body__item__title"><?php esc_html_e( 'Set the password', 'gi-toolkit' ); ?></div>
                        <div class="gi-toolkit__section__body__item__content">
                            <div class="gi-toolkit__input-password">
                                <input type="password" name="<?php echo esc_attr( $this->option_id . '[password]' ); ?>" value="<?php echo esc_attr( $password ); ?>">
                                <span class="eye-show"><?php echo file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/eye-show.svg' );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                                <span class="eye-hide"><?php echo file_get_contents( GI_TOOLKIT_PLUGIN_PATH . 'admin/svg/eye-hide.svg' );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                            </div>
                        </div>
                    </div>
                </div>
